...
class GetRopeIndexOp(OpsProxy):
    ...
    def _pytorch_fallback(
        ...
        """Compute 3D RoPE indices with the PyTorch fallback."""
...
class GetWindowIndexOp(OpsProxy):
    ...
    def _pytorch_fallback(
        ...
        """Compute window-attention indices with PyTorch."""
        del metadata
        ...
        for grid_t, grid_h, grid_w in grid_thw:
            ...
            index = torch.arange(grid_t * llm_grid_h * llm_grid_w, device=device).reshape(
...
get_rope_index = GetRopeIndexOp("get_rope_index")
get_window_index = GetWindowIndexOp("get_window_index")
