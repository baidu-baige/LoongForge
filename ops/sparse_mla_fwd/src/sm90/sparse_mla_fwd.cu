...
sparse_attn_fwd_kernel(__grid_constant__ const SparsePrefillParams1 params, __grid_constant__ const TmaParams tma_params) {
    ...
    if (warpgroup_idx == 0 || warpgroup_idx == 1) {
        ...
        // Write the rP matrix (the result of QK matrix multiplication) to global memory
        auto store_rP_to_global = [&](auto warpgroup_idx, int block_idx) {
            ...
            for (int row_idx = 0; row_idx < 2; ++row_idx) {
                ...
                for (int i = row_idx*2; i < size(rP); i += 4) {
                    ...
                    float2 p_vec = make_float2(rP(i), rP(i+1));
                    *reinterpret_cast<float2*>(p_out_ptr) = p_vec;
                }
            }
        };
        ...
        if (warpgroup_idx == 0) {
            ...
            for (int block_idx = 0; block_idx < num_topk_blocks; block_idx += 2) {
                ...
                mask_rP(Warpgroup0{});
                store_rP_to_global(Warpgroup0{}, block_idx);
            ...
            for (int block_idx = 0; block_idx < num_topk_blocks; block_idx += 2) {
                ...
                mask_rP(Warpgroup1{});
                store_rP_to_global(Warpgroup1{}, block_idx);
        ...
        auto load_token_indices = [&](int block_idx) {
            ...
            for (int buf_idx = 0; buf_idx < 2; ++buf_idx) {
                ...
                for (int local_row = 0; local_row < NUM_ROWS_PER_GROUP; ++local_row) {
                    ...
                    is_token_valid[buf_idx][local_row] = t >= 0 && t < params.s_kv && t <= max_kv_i;
                }
            }
        };
...
void run_fwd_kernel1(const SparsePrefillParams1& params) {
