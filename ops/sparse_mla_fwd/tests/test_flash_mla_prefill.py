...
def reference_torch(p: TestParam, t: Testcase, sm_scale: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    q_pos = torch.arange(p.s_q, device=indices.device).view(-1, 1) + p.q_start_index_s
    invalid_indices_mask = invalid_indices_mask | (indices > q_pos)
...
def run_test(p: TestParam) -> bool:
    ...
    def run_ans():
        return flash_mla_sparse_fwd(
            ...
            t.indices.squeeze(0),
            sm_scale=sm_scale,
            q_start_index_s=p.q_start_index_s
        )

    ans_out, ans_max_logits, ans_lse, ans_p_out = run_ans()
    torch.cuda.synchronize()
