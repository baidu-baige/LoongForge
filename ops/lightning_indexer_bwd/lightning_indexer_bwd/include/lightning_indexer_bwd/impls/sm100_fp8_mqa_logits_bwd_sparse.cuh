...
)
{
    ...
    using BF16_SWIZZLE_128B = cute::Swizzle<2, 4, 2>;
    ...
    static constexpr uint32_t SMEM_Q_BF16_SIZE_PER_STAGE       = BLOCK_Q * kNumHeads * kHeadDim * sizeof(cutlass::bfloat16_t);
    ...
    static constexpr uint32_t SMEM_D_LOGIT_SIZE_PER_STAGE      = BLOCK_Q * kNumHeads * BLOCK_KV * sizeof(cutlass::bfloat16_t);
    ...
    constexpr uint32_t kNumTmemCols = kNumTmemColsLogits + kNumTmemColsGradQ + kNumTmemColsGradKV; // kNumMathWarpGroups = 1
    ...
    if ( threadIdx.x >= (kNumMathThreads + kNumSpecializedThreads) )
    {
        ...
        constexpr auto q_bf16_layout = cute::make_layout(
            cute::make_shape(cute::Int<kNumHeads>{}, cute::Int<kHeadDim>{}),
            cute::make_stride(cute::Int<1>{}, cute::Int<kNumHeads>{})
        );
        ...
        while ( block_q_idx < num_q_blocks )
        {
            ...
            for ( uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; ++kv_block_idx )
            {
                ...
                // Load topk-indices
                uint32_t kv_idx = threadIdx.x % 128;
                volatile uint32_t* topk_index_ptr = topk_indices + block_q_idx * topk + kv_block_idx * BLOCK_KV + kv_idx;
                uint32_t topk_index = *topk_index_ptr;
                ...
                volatile float* kv_scale_ptr = kv_scales + global_kv_idx;
                smem_kv_scales[kv_stage_idx][kv_idx] = *kv_scale_ptr;
    ...
    else if ( is_umma_bf16_warp )
    {
        ...
        // [kHeadDim, BLOCK_KV] = [kHeadDim, BLOCK_Q * kNumHeads] @ [BLOCK_KV, BLOCK_Q * kNumHeads]^T, K = BLOCK_Q * kNumHeads = 64 BF16 (128B)
        ...
        while ( block_q_idx < num_q_blocks )
        {
            ...
            for ( uint32_t kv_block_idx = 0; kv_block_idx < num_kv_blocks; kv_block_idx++ )
            {
                ...
                #pragma unroll 1
                for ( uint32_t k = 0; k < (BLOCK_Q * kNumHeads) / UMMA_K_BF16; ++k )
                {
                    auto a_desc = make_umma_desc<cute::UMMA::Major::K, UMMA_M_BF16,         BLOCK_Q * kNumHeads, 128, true>(smem_q_bf16[q_stage_idx],                0, k * UMMA_K_BF16);
                    auto b_desc = make_umma_desc<cute::UMMA::Major::K, UMMA_N_BF16_GRAD_KV, BLOCK_Q * kNumHeads, 128, true>(smem_reshape_d_logit_bf16[kv_stage_idx], 0, k * UMMA_K_BF16);
                    UMMA_GRAD_KV::fma(a_desc, b_desc, kNumTmemColsLogits + kNumTmemColsGradQ, k, umma_desc_d_kv);
                }
    ...
    else if ( threadIdx.x < kNumMathThreads )
    {
        ...
        const auto& tmem_start_d_q    = __shfl_sync(0xffffffff, kNumTmemColsLogits, 0);
        const auto& tmem_start_d_kv   = __shfl_sync(0xffffffff, kNumTmemColsLogits + kNumTmemColsGradQ, 0);
        ...
        constexpr auto reshape_d_logit_bf16_layout = cute::make_layout(
            cute::make_shape(cute::Int<kNumHeads>{}, cute::Int<BLOCK_KV>{}),
            cute::make_stride(cute::Int<1>{}, cute::Int<kNumHeads>{})
        );
