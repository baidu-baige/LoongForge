# LoongForge MCore E2E Test Guide

This document explains how to use the automated E2E test scripts and configurations for the MCore CI suite. For the Torch VLA regression suite see [tests/torch/README.md](../torch/README.md).

## Directory Structure

- `tests/mcore/configs/`: Model configs run by default in CI.
- `tests/mcore/optional_configs/`: Optional test configs.
- `tests/mcore/main_start.sh`: Main test entry script.
- `tests/mcore/baseline/{default,optional}/<chip>/`: Per-model baseline JSON.
- `tests/mcore/unit/`: Standalone pytest unit tests.
...
Configure in `tests/mcore/main_start.sh`. **Uncomment** the desired mode and set variables as needed.
...
3. Run `bash tests/mcore/main_start.sh`. Baselines will be saved under `tests/mcore/baseline`.
