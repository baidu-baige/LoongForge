...
function summary_perf_metrics() {
    # Copy performance metrics data from container, log files are in container "/workspace/logs" directory

    metric_file=${scripts_root_path}/total_metric.csv
    echo "model_name,training_type_name,TP_PP_DP_ZERO,micro_batch_size,global_batch_size,seq_length,elapsed_time_per_iteration(ms),lr,initial_loss_scale,lm_loss,average_throughput(tokens/device/s)" > $metric_file

    collection_model_flag=""

    # Search for training type pretrain logs
    ...
    do
        ...
        # tokens/device/s=(global_batch_size*seq_length)/total_time/num_of_cards
