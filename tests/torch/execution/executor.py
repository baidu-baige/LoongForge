...
def _inject_model_env(env, model_name, logger):
    """Inject model-specific paths: TORCH_<MODEL NAME UPPERCASE>_<VAR> -> <VAR> (see config/env.sh)."""
    prefix = "TORCH_" + re.sub(r"[^0-9A-Za-z]", "_", model_name).upper() + "_"
...
def run_script(model_name, script_path, args, log_dir, logger):
    ...
    The baseline path is determined by reporting.baseline (TORCH_BASELINE_ROOT env var
    when set, otherwise the in-repo tests/torch/baseline default), so no repo-root
