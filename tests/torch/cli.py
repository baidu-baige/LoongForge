...
"""Torch regression executor (runs inside the container, manual trigger entry point).
...
Usage examples:
    python3 tests/torch/cli.py --models pi05 groot_n1_6
    python3 tests/torch/cli.py --auto_collect_baseline   # collect baseline on first run
    python3 tests/torch/cli.py --dry_run                 # only print commands, do not train
"""
...
# This file lives at tests/torch/cli.py; TORCH_ROOT is this suite's self-contained
# root (config/ + execution/ + reporting/). The torch suite has no dependency on the
# sibling tests/mcore/ suite.
...
def _ensure_env_loaded():
    ...
    directly (python3 tests/torch/cli.py ...), env.sh has not been sourced
    ...
    sourced (detected via the TORCH_CI_ROOT marker).
    """
    if os.environ.get("TORCH_CI_ROOT"):
        return
    env_sh = os.path.join(TORCH_ROOT, "config", "env.sh")
...
DEFAULT_LOG_ROOT = os.environ.get(
    "TORCH_LOG_ROOT", "/workspace/loongforge_torch_ci/logs")


def parse_args(available_models):
    parser = argparse.ArgumentParser(
        description="Torch regression runner. All flags below also accept an environment-variable form "
    ...
    parser.add_argument("--chip", default=env_chip,
                        help="Chip/baseline name; determines the baseline subdirectory "
                             "tests/torch/baseline/<chip>/. Env var: chip. Required.")
...
def main():
    manifest = load_manifest(TORCH_ROOT)
    ...
    if args.prepare:
        prepare_sh = os.path.join(TORCH_ROOT, "config", "prepare.sh")
    ...
    scripts_root = examples_dir(TORCH_ROOT)
