# Torch Regression Test Framework
...
This suite lives under [tests/torch/](.) and is fully self-contained (own config, executor, and
`baseline/`). The sibling MCore E2E suite is at [tests/mcore/](../mcore/); see
...
```
tests/torch/run.sh  (thin wrapper: source config/env.sh, then exec python3 cli.py "$@")
  └─ tests/torch/cli.py (execution layer)
       ...
       4. bash-execute the examples scripts serially one by one:
            inject OUTPUT_DIR / TENSORBOARD_DIR pointing to this run's log directory + model-specific env (TORCH_<MODEL>_<VAR>)
       ...
       6. aggregate and write results.json (default under ${TORCH_LOG_ROOT}/run_<timestamp>/; override with LOG_DIR)
...
- `TORCH_CI_ROOT` — default `/workspace/loongforge_torch_ci`; unified root directory for out-of-repo mutable state (data, logs, tools). Baselines are **no longer** derived from it by default.
- `LOCAL_VLA_ARTIFACTS_ROOT` — default `${TORCH_CI_ROOT}/vla_artifacts`; data/ckpt root, shared by download and training scripts
- `TORCH_LOG_ROOT` — default `${TORCH_CI_ROOT}/logs`; regression log/result root directory
- `TORCH_BASELINE_ROOT` — default **in-repo** at `tests/torch/baseline` (this suite owns its baselines; the sibling suite owns `tests/mcore/baseline/`). Override to point at a shared out-of-repo collection if multiple checkouts must share baselines.
- `BOS_VLA_ARTIFACTS_ROOT` — BOS source prefix used by `config/prepare.sh`
- `BCECMD_DIR` / `BCECMD` — bcecmd tool path, see file
- `TORCH_<MODEL NAME UPPERCASE>_<VAR>` — empty by default; model-specific required paths (e.g. fastwam's TOKENIZER_PATH / DATA_PATH), injected as `<VAR>` before running the corresponding script
...
> `${TORCH_CI_ROOT}/metrics_baseline/<chip>/` remains usable if you `export TORCH_BASELINE_ROOT=...`
> before running. Otherwise copy them into `tests/torch/baseline/<chip>/` once, or re-collect on
> the target machine with `--auto_collect_baseline`.

## Prerequisites

The following need to be prepared under `${TORCH_CI_ROOT}`:
...
- **Data and weights** `vla_artifacts/`: organized as `<family>/{models,datasets,tokenizers}`; the training scripts' default paths are derived from
  `LOCAL_VLA_ARTIFACTS_ROOT`. When files are missing or switching machines, use `bash tests/torch/config/prepare.sh` to sync from BOS.
...
- **baseline** `tests/torch/baseline/<chip>/<name>.json` (**critical**): missing baseline means a direct FAIL. Whichever chip you run on,
  first collect all models in the manifest on that chip:
  ```bash
  chip=a auto_collect_baseline=true bash tests/torch/run.sh
  ```
...
# Mutable per-run state (default outside the repo under ${TORCH_CI_ROOT}, see config/env.sh):
${TORCH_CI_ROOT}/logs/run_<timestamp>/    # per-run logs + results.json (TORCH_LOG_ROOT)
```
...
chip=a model_names="pi05_ddp" bash tests/torch/run.sh
bash tests/torch/run.sh --chip a --models pi05_ddp

# Run multiple scripts
chip=a model_names="pi05_ddp groot_n1_6_ddp" bash tests/torch/run.sh
bash tests/torch/run.sh --chip a --models pi05_ddp groot_n1_6_ddp

# First-time baseline collection (no comparison, write the current result into tests/torch/baseline/<chip>/)
chip=a auto_collect_baseline=true bash tests/torch/run.sh
...
| `chip` | `--chip` | **required** | Chip model, `a` / `p`; determines the baseline subdirectory `tests/torch/baseline/<chip>/` |
...
| `LOG_DIR` | `--log_dir` | `${TORCH_LOG_ROOT}/run_<ts>` | Log/result output directory |
...
- Missing `tests/torch/baseline/<chip>/<name>.json` -> **preflight refusal**: `cli.py` checks all selected
