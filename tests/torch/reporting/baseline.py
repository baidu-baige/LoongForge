...
baseline file: ${TORCH_BASELINE_ROOT}/<chip>/<model_name>.json

The default value of TORCH_BASELINE_ROOT resolves to the in-repo
`tests/torch/baseline` directory (computed from this file's location); export
TORCH_BASELINE_ROOT explicitly (typically via tests/torch/config/env.sh)
...
# baseline.py lives at tests/torch/reporting/baseline.py; two dirname() hops -> tests/torch,
# then baseline. This keeps the torch suite self-contained (its own baseline/ dir).
...
def _baseline_root():
    return os.environ.get("TORCH_BASELINE_ROOT") or _IN_REPO_BASELINE_ROOT


def baseline_path(chip, model_name):
    """Build the baseline JSON file path: ${TORCH_BASELINE_ROOT}/<chip>/<model_name>.json."""
