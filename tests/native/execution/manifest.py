...
"""Load native regression scripts from the split VLA/world example roots."""

import os

import yaml

# Path of the manifest file relative to the native suite root (tests/native).
MANIFEST_FILE = os.path.join("config", "scripts.yaml")


def examples_dir(native_root):
    """Return the shared examples root; manifest entries select vla/ or world/."""
    repo_root = os.path.dirname(os.path.dirname(native_root))
    return os.path.join(repo_root, "examples")


def load_manifest(native_root):
    """Return {model_name: script_relpath} in file order."""
    path = os.path.join(native_root, MANIFEST_FILE)
...
def list_models(native_root):
    return list(load_manifest(native_root))
