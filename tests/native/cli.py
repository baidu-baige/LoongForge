...
"""Native regression executor (runs inside the container, manual trigger entry point).
...
Usage examples:
    python3 tests/native/cli.py --models pi05 groot_n1_6
    python3 tests/native/cli.py --auto_collect_baseline   # collect baseline on first run
    python3 tests/native/cli.py --dry_run                 # only print commands, do not train
"""
...
# This file lives at tests/native/cli.py; NATIVE_ROOT is this suite's self-contained
# root (config/ + execution/ + reporting/). The native suite has no dependency on the
# sibling tests/llm_vlm/ suite.
NATIVE_ROOT = os.path.dirname(os.path.abspath(__file__))
...
def _ensure_env_loaded():
    ...
    directly (python3 tests/native/cli.py ...), env.sh has not been sourced
    ...
    sourced (detected via the NATIVE_CI_ROOT marker).
    """
    if os.environ.get("NATIVE_CI_ROOT"):
        return
    env_sh = os.path.join(NATIVE_ROOT, "config", "env.sh")
...
DEFAULT_LOG_ROOT = os.environ.get(
    "NATIVE_LOG_ROOT", "/workspace/loongforge_native_ci/logs")


def parse_args(available_models):
    parser = argparse.ArgumentParser(
        description="Native regression runner. All flags below also accept an environment-variable form "
    ...
    parser.add_argument("--chip", default=env_chip,
                        help="Chip/baseline name; determines the baseline subdirectory "
                             "tests/native/baseline/<chip>/. Env var: chip. Required.")
...
def main():
    manifest = load_manifest(NATIVE_ROOT)
    ...
    if args.prepare:
        prepare_sh = os.path.join(NATIVE_ROOT, "config", "prepare.sh")
    ...
    scripts_root = examples_dir(NATIVE_ROOT)
