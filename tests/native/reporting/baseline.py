...
baseline file: ${NATIVE_BASELINE_ROOT}/<chip>/<model_name>.json

The default value of NATIVE_BASELINE_ROOT resolves to the in-repo
`tests/native/baseline` directory (computed from this file's location); export
NATIVE_BASELINE_ROOT explicitly (typically via tests/native/config/env.sh)
...
# baseline.py lives at tests/native/reporting/baseline.py; two dirname() hops -> tests/native,
# then baseline. This keeps the native suite self-contained (its own baseline/ dir).
...
def _baseline_root():
    return os.environ.get("NATIVE_BASELINE_ROOT") or _IN_REPO_BASELINE_ROOT


def baseline_path(chip, model_name):
    """Build the baseline JSON file path: ${NATIVE_BASELINE_ROOT}/<chip>/<model_name>.json."""
