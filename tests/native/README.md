# Native Regression Test Framework

Manual regression tests for the `loongforge` subframework: **serially execute the training scripts under
`examples/{vla,world}` directly** -> parse metrics -> compare against the baseline within tolerance -> write `results.json`.

This suite lives under [tests/native/](.) and is fully self-contained (own config, executor, and
...
```
tests/native/run.sh  (thin wrapper: source config/env.sh, then exec python3 cli.py "$@")
  └─ tests/native/cli.py (execution layer)
       ...
       4. bash-execute the examples scripts serially one by one:
            inject OUTPUT_DIR / TENSORBOARD_DIR pointing to this run's log directory + model-specific env (NATIVE_<MODEL>_<VAR>)
       ...
       6. aggregate and write results.json (default under ${NATIVE_LOG_ROOT}/run_<timestamp>/; override with LOG_DIR)
...
- `NATIVE_CI_ROOT` — default `/ssd2/loongforge_native_ci`; unified root directory for out-of-repo mutable state (data, logs, tools). Baselines are **no longer** derived from it by default.
- `LOCAL_VLA_ARTIFACTS_ROOT` — default `${NATIVE_CI_ROOT}/vla_artifacts`; data/ckpt root, shared by download and training scripts
- `NATIVE_LOG_ROOT` — default `${NATIVE_CI_ROOT}/logs`; regression log/result root directory
- `NATIVE_BASELINE_ROOT` — default **in-repo** at `tests/native/baseline` (this suite owns its baselines; the sibling suite owns `tests/llm_vlm/baseline/`). Override to point at a shared out-of-repo collection if multiple checkouts must share baselines.
- `BOS_VLA_ARTIFACTS_ROOT` — BOS source prefix used by `config/prepare.sh`
- `BCECMD_DIR` / `BCECMD` — bcecmd tool path, see file
- `NATIVE_<MODEL NAME UPPERCASE>_<VAR>` — empty by default; model-specific required paths (e.g. fastwam's TOKENIZER_PATH / DATA_PATH), injected as `<VAR>` before running the corresponding script
...
> `${NATIVE_CI_ROOT}/metrics_baseline/<chip>/` remains usable if you `export NATIVE_BASELINE_ROOT=...`
> before running. Otherwise copy them into `tests/native/baseline/<chip>/` once, or re-collect on
> the target machine with `--auto_collect_baseline`.

## Prerequisites

The following need to be prepared under `${NATIVE_CI_ROOT}`:
...
- **Data and weights** `vla_artifacts/`: organized as `<family>/{models,datasets,tokenizers}`; the training scripts' default paths are derived from
  `LOCAL_VLA_ARTIFACTS_ROOT`. When files are missing or switching machines, use `bash tests/native/config/prepare.sh` to sync from BOS.
...
- **baseline** `tests/native/baseline/<chip>/<name>.json` (**critical**): missing baseline means a direct FAIL. Whichever chip you run on,
  first collect all models in the manifest on that chip:
  ```bash
  chip=a auto_collect_baseline=true bash tests/native/run.sh
  ```
...
The regression targets are defined by the `config/scripts.yaml` manifest, a YAML mapping of `<name>: <script path relative to examples/{vla,world}>`.
...
To add a regression model: add an executable training script under `examples/{vla,world}/`, then add a line in
...
│   ├── scripts.yaml            #   regression script manifest: name → script under examples/{vla,world}
...
tests/native/baseline/<chip>/<name>.json

# Mutable per-run state (default outside the repo under ${NATIVE_CI_ROOT}, see config/env.sh):
${NATIVE_CI_ROOT}/logs/run_<timestamp>/    # per-run logs + results.json (NATIVE_LOG_ROOT)
```
...
chip=a model_names="pi05_ddp" bash tests/native/run.sh
bash tests/native/run.sh --chip a --models pi05_ddp

# Run multiple scripts
chip=a model_names="pi05_ddp groot_n1_6_ddp" bash tests/native/run.sh
bash tests/native/run.sh --chip a --models pi05_ddp groot_n1_6_ddp

# First-time baseline collection (no comparison, write the current result into tests/native/baseline/<chip>/)
chip=a auto_collect_baseline=true bash tests/native/run.sh
...
| `chip` | `--chip` | **required** | Chip model, `a` / `p`; determines the baseline subdirectory `tests/native/baseline/<chip>/` |
...
| `LOG_DIR` | `--log_dir` | `${NATIVE_LOG_ROOT}/run_<ts>` | Log/result output directory |
...
- Missing `tests/native/baseline/<chip>/<name>.json` -> **preflight refusal**: `cli.py` checks all selected
