...
from aiak_training_omni.train.sft.utils import build_sft_data_collator
from aiak_training_omni.data.multimodal.dataloader_provider import (
    get_train_dataset,
    get_train_loader
)
from aiak_training_omni.data.multimodal.llava_ov_task_encoder import LLavaOv15TaskEncoder
...
def get_batch(data_iterator):
    ...
    if data_iterator is not None and mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if isinstance(data.get('tokens'), torch.Tensor):
            ...
            if data['tokens'].dtype != torch.long:
                print(
                    f"[WARN] tokens dtype {orig_dtype} -> force cast to torch.long; "
                    f"shape={tuple(data['tokens'].shape)}"
                )
    ...
    if cu_lengths.shape == torch.Size([1, 1]):
        for i in range(attn_mask.shape[0]):
            loss_mask[i, (~attn_mask[i]).sum() - 1] = 0
    else:
...
def train_valid_test_dataset_provider(train_val_test_num_samples):
    ...
    args = get_args()
    task_encoder = LLavaOv15TaskEncoder(args)
