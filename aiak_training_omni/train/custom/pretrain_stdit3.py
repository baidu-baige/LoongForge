...
from aiak_training_omni.data.video.latent_dataset import VariableLatentDataset
from aiak_training_omni.data.video.video_sampler import VariableVideoBatchSampler
...
SUPPORTED_MODELS = [
    VideoLanguageModelFamilies.STDIT3,
]
...
def get_batch(data_iterator, diffusion):
    """Generate a batch"""
...
def rectified_flow():
    """Build the rectified flow scheduler."""
...
def loss_func(
    ...
    loss = mean_flat((model_output - (video - target)).pow(2)).mean()
...
def train_valid_test_datasets_provider(diffusion, train_val_test_num_samples):
    ...
    dataset = VariableLatentDataset(
        ...
        frame_size_divisible_by=mpu.get_context_parallel_world_size(),
        max_text_length=args.max_text_length,
    )

    batch_sampler = VariableVideoBatchSampler(
        dataset,
        args.stdit_bucket_config,
