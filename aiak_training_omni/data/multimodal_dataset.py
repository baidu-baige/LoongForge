...
class BaseDataset(ABC, torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_name = os.path.join(
            self.label_dir, self.filenames[idx].replace(".jpg", ".json")
        )
        ...
        input_data = self.build_conversation_input_ids(
            query=query, history=history, images=[image], answer=response
        )
        ...
        input_data["attention_mask"] = pad_to_len(
            input_data["attention_mask"], self.max_length, pad_value=0
        )
        input_data["token_type_ids"] = pad_to_len(
            input_data["token_type_ids"], self.max_length, pad_value=0
        )
        ...
        input_data["position_ids"] = _build_position_ids(
            input_data["token_type_ids"], input_data["attention_mask"]
        )
        input_data["attention_mask"] = (
            input_data["attention_mask"].logical_not().unsqueeze(0).unsqueeze(0)
        )  # [1, 1, s]
    ...
    def build_conversation_input_ids(
        ...
        *,
        ...
        if images is not None and len(images) == 1:
            ...
            # language
            vision_token_num = (self.image_size[0] // self.patch_size // 2) * (
        ...
        if answer is not None:
            labels = [
                -100 for _ in range(len(input_ids) - len(answer_ids))
            ] + answer_ids
...
class ConversationDataset(BaseDataset):
    ...
    def get_texts(self, label_data):
        ...
        history = (
            [
                (
                    label_data["conversations"][(sampled_round_id - 1) * 2]["content"],
                    label_data["conversations"][(sampled_round_id - 1) * 2 + 1][
                        "content"
                    ],
                )
            ]
            if (sampled_round_id > 0 and random.random() > 0.5)
            else None
        )
