...
def build_eagle_load_kwargs(
    ...
    if "attn_implementation" not in loading_kwargs:
...
def resolve_eagle_local_path(model_name: str, current_file: str) -> str | None:
    """Resolve best-effort local Eagle model path from explicit model/cache paths."""
    ...
    if cache_root:
        ...
        cache_candidates = [
            os.path.join(cache_root, sanitized),
            os.path.join(cache_root, f"models--{sanitized}"),
            os.path.join(cache_root, model_name.rsplit("/", 1)[-1]),
        ]
