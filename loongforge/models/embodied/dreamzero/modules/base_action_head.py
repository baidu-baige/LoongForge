...
"""ActionHead abstract base class.

Provides the abstract surface consumed by ``WANPolicyHead``:
- ``forward(backbone_output, action_input) -> BatchFeature``
- ``get_action(...)`` defaulting to ``forward`` for inference
- ``prepare_input(batch)`` (no-op by default)
- ``set_override_kwargs(**kwargs)`` (mirrors attributes onto ``self.config``)
"""
