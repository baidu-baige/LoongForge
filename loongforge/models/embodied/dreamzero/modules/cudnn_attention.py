...
#
# cuDNN attention helper. The torch.library custom_op namespace uses
# ``loongforge::`` to avoid collisions with other installed attention helpers.
...
class DotProductAttention(torch.nn.Module):
    """cuDNN-backed dot-product attention wrapper for TransformerEngine."""
...
class FusedAttention(torch.nn.Module):
    """Fused attention module dispatching to the custom fused_attn op."""
...
@torch.library.custom_op("loongforge::fused_attn", mutates_args=())
def fused_attn(
    is_training: bool,
    max_seqlen_q: int,
    max_seqlen_kv: int,
    ...
    """Custom op wrapping TransformerEngine's forward fused-attention kernel."""
...
def _(
    ...
    """Fake (meta) implementation of ``fused_attn`` for tracing/torch.compile."""
    return [
...
def fused_attn_bwd_setup_context(ctx, inputs, output) -> None:
    """Save tensors and metadata on ``ctx`` for the fused_attn backward pass."""
    (
        _,
        max_seqlen_q,
        max_seqlen_kv,
...
@torch.library.custom_op("loongforge::fused_attn_bwd_op", mutates_args=())
def fused_attn_bwd_op(
    ...
    """Custom op wrapping TransformerEngine's backward fused-attention kernel."""
    args = (
        max_seqlen_q,
        max_seqlen_kv,
        attn_scale,
...
def fused_attn_bwd_impl(ctx, grad):
    """Run the fused-attention backward implementation for saved forward context."""
    d_out, _, _ = grad
