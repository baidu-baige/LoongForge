...
"""Schedulers used by FastWAM action diffusion models."""
...
class WanContinuousFlowMatchScheduler:
    ...
    def __init__(self, num_train_timesteps: int = 1000, shift: float = 5.0, eps: float = 1e-10):
        """Initialize scheduler constants for training and inference."""
    ...
    def _phi(u: torch.Tensor, shift: float) -> torch.Tensor:
        """Map uniform samples to shifted sigma values."""
        return shift * u / (1.0 + (shift - 1.0) * u)

    def _precompute_training_weight_stats(self) -> tuple[float, float]:
        """Precompute normalization constants for timestep training weights."""
    ...
    def training_weight(self, timestep: torch.Tensor) -> torch.Tensor:
        """Return the normalized training weight for each timestep."""
    ...
    def build_inference_schedule(
        ...
        """Build inference timesteps and integration deltas."""
    ...
    def step(model_output: torch.Tensor, delta: torch.Tensor, sample: torch.Tensor) -> torch.Tensor:
        """Apply one Euler integration step to the current sample."""
