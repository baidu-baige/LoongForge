...
from loongforge.data.embodied.datasets.cosmos3.transforms.cosmos3_action_transform import VIDEO_RES_SIZE_INFO
...
def sync_model_states(
    ...
    log.info(
        f"Synchronizing model states from rank {src} to all ranks in "
        f"process group {get_process_group_ranks(process_group)}."
    )
...
class Resample(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=None):  # x: [B,C,T,H,W]
        """forward."""
        if feat_idx is None:
            feat_idx = [0]
        b, c, t, h, w = x.size()
...
class WanVAE:
    ...
    def encode(self, videos: torch.Tensor) -> torch.Tensor:
        ...
        :meth:`Wan2pt2VAEInterface.compile_encode`)
        are dispatched from inside ``WanVAEModule.encode`` at the per-chunk level,
...
class _ChunkEncodeForAOT(torch.nn.Module):
    """Wrapper around ``WanVAEModule._encode_chunk_impl`` for ``torch.export``.
...
class Wan2pt2VAEInterface():
    ...
    def compile_encode(
        ...
        Compiles ``WanVAEModule._encode_chunk_impl`` for each
        ...
        **Shared weights** — packages are compiled with
        ...
        for dispatch by ``_run_chunk`` inside ``WanVAEModule.encode``.
        ...
        wanvae_model = wanvae.model  # WanVAEModule (nn.Module)
