...
#
# Modified from HuggingFace transformers (Siglip2) under the Apache-2.0 License.
...
def flash_attention_forward_for_packing(
    ...
    **kwargs,
    ...
    if _is_cuda_capturing():
        ...
        # Keep eager training identical to LeRobot. Also refresh the cached
        # tensors for a later CUDA graph capture.
        cu_seqlens = F.pad(
            torch.cumsum(torch.tensor(seq_len_list, device=query.device, dtype=torch.int32), dim=0),
            (1, 0),
        )
        cu_seqlens = cu_seqlens.to(torch.int32)
        max_seq_len = max(seq_len_list)
        module._cu_seqlens_cache = cu_seqlens.detach()
        module._max_seq_len_cache = max_seq_len
...
class Siglip2VisionEmbeddings(nn.Module):
    ...
    def resize_positional_embeddings(
        ...
        for i in range(batch_size):
            ...
            antialias = not _is_cuda_capturing()
    ...
    def get_spatial_shapes(self, bchw_list: list[torch.Tensor]) -> torch.Tensor:
        ...
        if target_device.type == "cuda":
            cached_hw = hw_tensor.to(target_device)
            if (hasattr(self, '_cached_spatial_shapes') and
                    self._cached_spatial_shapes is not None and
                    self._cached_spatial_shapes.shape == cached_hw.shape):
                self._cached_spatial_shapes.copy_(cached_hw)
            else:
                self._cached_spatial_shapes = cached_hw
        return hw_tensor
        ...
        if _is_cuda_capturing():
            ...
            if resized_positional_embeddings.device.type == "cuda":
                if (hasattr(self, '_cached_resized_pos_emb') and
                    ...
                    self._cached_resized_pos_emb = resized_positional_embeddings.detach().clone()
