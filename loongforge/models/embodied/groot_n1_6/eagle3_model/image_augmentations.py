"""Image augmentation utilities for Gr00tN1d6.
...
class FractionalRandomCrop(A.DualTransform):
    ...
    def __init__(
        ...
        """Initialize FractionalRandomCrop transform."""
    ...
    def get_params_dependent_on_data(self, params, data) -> dict[str, tuple[int, int, int, int]]:
        """Get random crop coordinates based on image dimensions."""
    ...
    def get_transform_init_args_names(self) -> tuple[str, ...]:
        """Return the names of the arguments used to initialize the transform."""
...
class FractionalCenterCrop(A.DualTransform):
    ...
    def __init__(
        ...
        """Initialize FractionalCenterCrop transform."""
        super().__init__(p=p, always_apply=always_apply)
        if not 0.0 < crop_fraction <= 1.0:
            raise ValueError("crop_fraction must be between 0.0 and 1.0")
        self.crop_fraction = crop_fraction
...
class LetterBoxPad(A.DualTransform):
    ...
    def __init__(self, p: float = 1.0, always_apply: bool | None = None):
        """Initialize LetterBoxPad transform."""
        super().__init__(p=p, always_apply=always_apply)
    ...
    def apply(
        ...
        **params,
        ...
        Args:
            img: Input image.
            pad_top: Top padding size.
            pad_bottom: Bottom padding size.
    ...
    def get_params_dependent_on_data(self, params, data) -> dict[str, int]:
        """
        Get padding parameters based on image dimensions.

        Args:
            params: Dictionary containing image shape.
            data: Unused parameter kept for compatibility.

        Returns:
            Dictionary of padding parameters.
        """
        h, w = params["shape"][:2]
        if h == w:
    ...
    def get_transform_init_args_names(self) -> tuple[str, ...]:
        """
        Get initialization arguments names for transform.

        Returns:
            Empty tuple (no initialization arguments needed).
        """
        return ()
