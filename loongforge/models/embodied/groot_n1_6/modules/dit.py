...
class TimestepEncoder(nn.Module):
    ...
    def forward(self, timesteps):
        """Encode scalar timesteps into dense conditioning embeddings."""
...
class AdaLayerNorm(nn.Module):
    ...
    def forward(
        ...
        """Apply adaptive layer normalization conditioned on timestep embedding."""
...
class DiT(ModelMixin, ConfigMixin):
    """Diffusion Transformer backbone used in Gr00t N1.6.

    The module stacks multiple transformer blocks with timestep conditioning and
    optional interleaved self-attention/cross-attention. It takes latent token
    features and conditioning features, then predicts transformed output tokens
    through a normalized projection head.

    Notes:
        - Inherits ``ModelMixin`` and ``ConfigMixin`` from diffusers.
    ...
    def forward(
        ...
        """Forward pass of DiT with optional collection of per-layer hidden states."""
        # Encode timesteps
...
class AlternateVLDiT(DiT):
    """Vision-language DiT variant with alternating token-group cross-attention.

    This model extends :class:`DiT` and splits cross-attention conditioning tokens
    into two groups using masks: image tokens and non-image (e.g., text) tokens.
    During cross-attention layers, it alternates which group is attended based on
    ``attend_text_every_n_blocks``, while keeping interleaved self-attention blocks.

    Intended use:
        Improve modality balancing by preventing every cross-attention block from
        attending to the same token subset.
    """
    ...
    def forward(
        ...
        non_image_attention_mask = torch.logical_not(image_mask) & backbone_attention_mask
...
class SelfAttentionTransformer(ModelMixin, ConfigMixin):
    """Stacked self-attention transformer encoder used for latent refinement.

    This module builds a sequence of :class:`BasicTransformerBlock` layers that
    operate in pure self-attention mode (no cross-attention conditioning). It is
    intended for scenarios where token features are refined using only internal
