...
def discover_transform_builders() -> None:
    ...
    for group in ("robotics", "world"):
        group_dir = datasets_dir / group
        for module_info in pkgutil.iter_modules([str(group_dir)]):
            if not module_info.ispkg or module_info.name.startswith("_"):
                continue
            if (group_dir / module_info.name / "transforms").is_dir():
                importlib.import_module(
                    f"loongforge.datasets.{group}.{module_info.name}.transforms"
                )
