...
"""DreamZero co-training data transform and collation utilities.
...
``DreamTransform``/``DefaultDataCollator`` used to prepare DreamZero batches
(video, state, action, language) for training and inference.
"""
...
from loongforge.datasets.common.transforms.collator import (
    _move_to_device,
    _pin_memory,
...
@register_preprocessor("dreamzero")
class DefaultDataCollator(DataCollatorMixin):
    ...
    def from_config(cls, model_cfg, data_cfg, training_args=None, dataset_stats=None, dataset=None):
        """Build the DreamZero collator from typed configs (preprocessor registry entry).
        ...
        """
        from loongforge.datasets.world.dreamzero.dataset.modality_configs import (
...
class DreamTransform(InvertibleModalityTransform):
    """Prepares DreamZero batches (video, state, action, language) for training/inference."""
    ...
    precomputed_first_frame_latents_key: str = "first_frame_latents"
    ...
    def check_keys_and_batch_size(self, data):
        """Group batch keys by modality and infer whether ``data`` is batched."""
        grouped_keys = {}
    ...
    def apply_single(self, data: dict) -> dict:
        """Transform a single unbatched sample into the DreamZero training format."""
        transformed_data = {}
        ...
        if self.training:
            ...
            if is_detection_instance:
                ...
                if action_dim < 3:
                    raise ValueError(
                        "GR1_UNIFIED_SEGMENTATION requires action_dim >= 3 "
                        f"for segmentation target/mask slicing, got {action_dim}"
                    )
        ...
        if self.precomputed_video_latents:
            latent_key = self.precomputed_video_latents_key
            if latent_key in data:
                transformed_data[latent_key] = data[latent_key]

        if self.precomputed_first_frame_latents:
            first_frame_key = self.precomputed_first_frame_latents_key
            if first_frame_key in data:
                transformed_data[first_frame_key] = data[first_frame_key]
    ...
    def apply_batch(self, data: dict, batch_size: int) -> dict:
        """Split a batched dict along the batch dim and apply per-sample transform."""
    ...
    def unapply(self, data: dict) -> dict:
        """Return the data unchanged so ConcatTransform can split the values."""
