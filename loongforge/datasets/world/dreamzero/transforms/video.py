...
"""Video modality transforms for DreamZero datasets."""
...
class VideoTransform(ModalityTransform):
    """Base class for backend-specific video modality transforms."""
    ...
    _INTERPOLATION_MAP: ClassVar[dict[str, dict[str, Any]]] = PrivateAttr(
        {
            ...
            "lanczos4": {
                ...
                # Torchvision does not specify filter size and may differ from 4x4.
    ...
    def train_transform(self) -> Callable:
        """Return the configured training transform."""
    ...
    def eval_transform(self) -> Callable | None:
        """Return the configured evaluation transform, if any."""
    ...
    def original_resolutions(self) -> dict[str, tuple[int, int]]:
        """Return original resolutions keyed by video modality."""
    ...
    def check_input(self, data: dict[str, Any]):
        """Validate input container types and video tensor ranks."""
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        for key in self.apply_to:
            ...
            if sub_key in dataset_metadata.modalities.video:
                ...
                raise ValueError(
                    f"Video key {sub_key} not found in dataset metadata. "
                    f"Available keys: {dataset_metadata.modalities.video.keys()}"
                )
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable | None:
        """Build the transform for the requested execution mode."""
...
class VideoCrop(VideoTransform):
    """Crop videos randomly for training and centrally for evaluation."""
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        ...
        Returns:
            Callable: Random crop in training mode or center crop in evaluation mode.
        """
...
class VideoResize(VideoTransform):
    """Resize videos to a fixed spatial resolution."""
    ...
    def validate_interpolation(cls, v):
        """Validate the configured interpolation mode."""
...
class VideoColorJitter(VideoTransform):
    """Apply configurable color jitter during training."""
...
class VideoToTensor(VideoTransform):
    """Convert uint8 numpy video frames to float tensors."""
    ...
    def check_input(self, data: dict):
        ...
        for key in self.apply_to:
            ...
            assert (
                ...
                f"expected {expected_resolution}. Full shape: {data[key].shape}"
            )
...
class VideoToNumpy(VideoTransform):
    """Convert normalized video tensors back to uint8 numpy arrays."""
