...
from loongforge.datasets.text.minicpm_v_4_6_plugin import MiniCPMV46Plugin
from loongforge.datasets.multimodal.vlm_task_encoder import (
    IMAGE_TOKEN,
...
class MiniCPMV46TaskEncoder(VLMTaskEncoder):
    ...
    def process_images(self, samples):
        """Concatenate MiniCPM packed image columns across batch samples."""
        imgs = [img for sample in samples if sample.imgs is not None for img in sample.imgs]
        if not imgs:
            return torch.tensor([[0]], dtype=torch.float32)
        if all(img.dim() == 4 and img.shape[0] == 1 for img in imgs):
