...
from . import data_adaptive as _data_adaptive_mod
from . import data_process as _data_process_mod
from .data_utils import merge_list
...
from .utils.io_utils import image_info_2_hash
import logging
    ...
    def _example_to_feature(
        ...
        **kwargs,
        ...
        try:
            ...
            adaptiver = getattr(_data_adaptive_mod, "Adaptive")(
        ...
        if len(ids) <= self.max_seq_length:
            ...
            if split_token_num < min_num:
                raise ValueError(
                    ...
                    f"  - image token count (id == {self.image_token_id})"
                    f": {sum(np.array(ids) == self.image_token_id)}\n"
