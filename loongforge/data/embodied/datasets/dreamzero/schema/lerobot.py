...
"""LeRobot and trainable-dataset schema definitions for DreamZero."""
...
class LeRobotModalityMetadata(BaseModel):
    ...
    def check_original_keys(self):
        """Fill in default original keys for state/action/video/annotation entries."""
    ...
    def get_key_meta(self, key: str) -> LeRobotModalityField:
        ...
        if modality == "state":
            ...
            if subkey not in self.action:
                raise ValueError(
                    f"Key: {key}, action key {subkey} not found in metadata, "
                    f"available action keys: {self.action.keys()}"
                )
            ...
            if subkey not in self.annotation:
                raise ValueError(
                    ...
                    f"available annotation keys: {self.annotation.keys()}"
                )
...
class DatasetStatisticalValues(BaseModel):
    """Per-dimension statistical values (max/min/mean/std/percentiles)."""
    ...
    def serialize_ndarray(self, v: np.ndarray) -> list:
        """Serialize ndarray fields to lists for JSON output."""
        return v.tolist()  # type: ignore

    @field_validator("*", mode="before")
    def validate_ndarray(cls, v) -> np.ndarray:
        """Coerce incoming field values to numpy arrays."""
        return np.array(v)


class DatasetStatistics(BaseModel):
    """Statistics of the state and action modalities."""
...
class StateActionMetadata(BaseModel):
    """Metadata describing a state or action split (shape, rotation, continuity)."""
...
class DatasetModalities(BaseModel):
    """Metadata of the video, state, and action modalities for a dataset."""
