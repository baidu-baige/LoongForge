...
"""Concatenation transforms for DreamZero dataset modalities."""
...
class ConcatTransform(InvertibleModalityTransform):
    ...
    apply_to: list[str] = Field(
        ...
        description="Inherited modality selector; concatenation uses explicit order fields.",
    )
    ...
    def model_dump(self, *args, **kwargs):
        """Serialize only the fields required by concatenation."""
    ...
    def get_modality_metadata(self, key: str) -> StateActionMetadata:
        """Return metadata for a state or action key."""
    ...
    def is_rotation_key(self, key: str) -> bool:
        """Return whether a key contains a rotation representation."""
        modality_config = self.get_modality_metadata(key)
