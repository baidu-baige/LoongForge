...
class MiniCPMV46ImageProcessor:
    ...
    def from_pretrained(cls, pretrained_path: str | Path, **overrides: Any) -> "MiniCPMV46ImageProcessor":
        """Load processor settings from a MiniCPM preprocessor config."""
    ...
    def find_best_resize(
        ...
        """Pick a resize target that respects the MiniCPM patch grid."""
    ...
    def get_refine_size(
        ...
        """Return the refined grid size used for sliced MiniCPM images."""
        height, width = image_size
        grid_y, grid_x = grid
    ...
    def get_sliced_grid(
        ...
        """Choose the best slice layout for a potentially large image."""
    ...
    def _to_tensor(image: Image.Image | np.ndarray | torch.Tensor) -> torch.Tensor:
        """Convert supported image inputs into a 3-channel tensor."""
    ...
    def _resize(image: torch.Tensor, height: int, width: int) -> torch.Tensor:
        """Resize an image tensor with MiniCPM's interpolation settings."""
    ...
    def _normalize(self, image: torch.Tensor) -> torch.Tensor:
        """Normalize a MiniCPM image tensor using the configured statistics."""
    ...
    def _reshape_by_patch(image: torch.Tensor, patch_size: int) -> torch.Tensor:
        """Repack a resized image into MiniCPM patch-major columns."""
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        """Preprocess one or more images into packed MiniCPM tensors."""
