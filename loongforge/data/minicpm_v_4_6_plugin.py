...
class MiniCPMV46Plugin(MMPlugin):
    ...
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        """Convert images to RGB before MiniCPM-specific preprocessing."""
        del kwargs
        return image.convert("RGB") if image.mode != "RGB" else image

    @staticmethod
    def _get_image_processor(processor):
        """Return a usable image processor, constructing a fallback if needed."""
    ...
    def _target_sizes_to_grid_thw(target_sizes):
        """Convert MiniCPM target sizes to `[T, H, W]` grid metadata."""
        import torch
    ...
    def _get_mm_inputs(self, images, videos, processor) -> Dict[str, "torch.Tensor"]:
        """Build the MiniCPM image inputs required by the chat template."""
    ...
    def _token_count(target_size, divisor: int) -> int:
        """Count MiniCPM image tokens for a grid using the configured divisor."""
    ...
    def _get_token_divisor(self, processor) -> int:
        """Select the token divisor that matches the current image processor."""
        image_processor = getattr(processor, "image_processor", None)
    ...
    def _image_placeholder(
        ...
        """Render a single MiniCPM image placeholder string."""
        placeholder = (
    ...
    def _build_image_placeholders(
        ...
        *,
        ...
        """Build per-image placeholders that mirror MiniCPM packed slices."""
    ...
    def process_messages(
        ...
        """Expand MiniCPM placeholders inside message content."""
        self._reject_videos(videos)
    ...
    def get_mm_inputs(
        ...
        """Expose MiniCPM image inputs for the standard multimodal pipeline."""
