...
from loongforge.engine.native.distributed.utils import is_dmuon_model, unwrap_dmuon_optimizer, unwrap_model
from loongforge.checkpoint.manifest import read_native_metadata, write_native_metadata
...
def save_checkpoint(
    ...
    meta = NativeCheckpointMetadata(step, epoch, save_format, ctx.world_size, use_lora)
    ...
    if ctx.is_main:
        write_native_metadata(path, meta)
...
def flush_pending_save(ctx: Optional[DistributedContext] = None):
    ...
    try:
        ...
        if pending["is_main"]:
            write_native_metadata(pending["path"], pending["meta"])
...
def load_pretrained(model: nn.Module, checkpoint_path: str, ctx: DistributedContext) -> nn.Module:
    ...
    ``python -m tools.dist_checkpoint.dcp_to_safetensors`` first to produce a single-file version.
    """
...
def get_latest_checkpoint(
    ...
    meta = read_native_metadata(ckpt_dir)
    return ckpt_dir, int(meta.completed_steps), int(meta.epoch)
...
def _save_legacy(
    ...
    if ctx.is_main:
        if use_lora:
            from loongforge.engine.native.lora import (
...
def _write_adapter_meta(path: str, training_args, model_cfg) -> None:
    ...
    from loongforge.engine.native.lora import adapter_meta
...
def _resolve_file(checkpoint_path: str) -> str:
    if os.path.isdir(checkpoint_path):
        ...
        raise FileNotFoundError(
            ...
            f"(if this is a DCP checkpoint, run python -m tools.dist_checkpoint.dcp_to_safetensors first)."
        )
