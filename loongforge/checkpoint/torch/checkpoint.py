...
def save_checkpoint(
    ...
    meta = TorchCheckpointMetadata(step, epoch, save_format, ctx.world_size, use_lora)
    ...
    if ctx.is_main:
        write_torch_metadata(path, meta)
...
def flush_pending_save(ctx: Optional[DistributedContext] = None):
    ...
    try:
        ...
        if pending["is_main"]:
            write_torch_metadata(pending["path"], pending["meta"])
...
def get_latest_checkpoint(
    ...
    meta = read_torch_metadata(ckpt_dir)
...
def _save_legacy(
    ...
    if ctx.is_main:
        if use_lora:
            from loongforge.engine.torch.lora import (
...
def _write_adapter_meta(path: str, training_args, model_cfg) -> None:
    ...
    from loongforge.engine.torch.lora import adapter_meta
