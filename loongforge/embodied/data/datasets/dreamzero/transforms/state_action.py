...
"""State and action transforms used by DreamZero datasets."""
...
class RotationTransform:
    """Convert between supported rotation representations.

    Adapted from diffusion-policy's ``rotation_transformer.py``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Convert rotations from the source to target representation."""
        assert isinstance(
            x, torch.Tensor
    ...
    def inverse(self, x: torch.Tensor) -> torch.Tensor:
        """Convert rotations from the target back to source representation."""
        assert isinstance(
            x, torch.Tensor
        ), f"Unexpected input type: {type(x)}. Expected type: {torch.Tensor}"
...
class Normalizer:
    """Normalize and denormalize state or action tensors."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Normalize a tensor with the configured statistics."""
        assert isinstance(
            x, torch.Tensor
        ), f"Unexpected input type: {type(x)}. Expected type: {torch.Tensor}"
        ...
        if self.mode == "q99":
            ...
            if not (torch.all(normalized <= 1 + 1e-5) and torch.all(normalized >= -1 - 1e-5)):
                logger.warning(
                    "Normalization out of range: shape=%s, max=%s, min=%s",
                    x.shape,
                    torch.max(normalized),
                    torch.min(normalized),
                )
    ...
    def inverse(self, x: torch.Tensor) -> torch.Tensor:
        """Restore a tensor from the configured normalization."""
        assert isinstance(
            x, torch.Tensor
        ), f"Unexpected input type: {type(x)}. Expected type: {torch.Tensor}"
        if self.mode == "q99":
...
class StateActionToTensor(InvertibleModalityTransform):
    ...
    def model_dump(self, *args, **kwargs):
        """Serialize only the fields required by this transform."""
    ...
    def validate_dtypes(cls, v):
        """Resolve serialized dtype names to dtype objects."""
...
class StateActionTransform(InvertibleModalityTransform):
    ...
    def model_dump(self, *args, **kwargs):
        """Serialize the configurable state-action transform fields."""
        if kwargs.get("mode", "python") == "json":
    ...
    def validate_modality_metadata(cls, v):
        """Validate and normalize state-action modality metadata."""
    ...
    def validate_normalization_statistics(self):
        """Validate statistics required by each normalization mode."""
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        for key in self.target_rotations:
            ...
            # Initialize a transformer when source and target representations differ.
            if from_rep != to_rep:
        ...
        for key in self.normalization_modes:
            ...
            # Convert absolute rotations to the target representation and normalize
            # them with min_max mode, whose bounds come from the representation.
        ...
        for key in self.apply_to:
            ...
            if key not in self._input_dtypes:
                ...
                assert (
                    ...
                    f"input dtype: {data[key].dtype}, "
                    f"expected dtype: {self._input_dtypes[key]}"
                )
            # Rotate the state
            state = data[key]
            if key in self._rotation_transformers:
