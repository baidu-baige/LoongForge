...
#
# Vendored from giga-brain-0 (https://github.com/open-gigaai/giga-brain-0)
# giga_brain_0/giga_brain_0_transforms.py. The transform pipeline body
# (__call__ and its call order: delta-action -> normalize -> tokenize ->
# pad -> image transform -> trajectory transform) is unmodified so per-sample
# preprocessing numerics match the reference exactly.
#
# Deviations from upstream, both purely about registration (no numeric
# effect):
#   1. Imports from the local ``giga_brain_0_utils`` (vendored alongside this
#      file) instead of the external ``giga_models`` package.
#   2. Drops the ``@TRANSFORMS.register`` decorator from ``giga_train`` — this
#      model plugs into LoongForge's own transform-builder registry instead
#      (see ``transforms/giga_brain_transform.py::build_giga_brain_transforms``).
...
from loongforge.embodied.data.datasets.giga_brain.giga_brain_0_utils import (
    DeltaActions,
    ImageTransform,
    Normalize,
    PadStatesAndActions,
...
class RobotType(StrEnum):
    ...
    # AgiBot-World Alpha (A2D dual-arm robot) shares the AgiBot G1 embodiment
    # (state=20, action=22, same delta mask). See scripts/convert_to_lerobot.py.
    A2D = 'a2d'
...
robot_type_mapping: dict[RobotType, EmbodimentId] = {
    ...
    RobotType.A2D: EmbodimentId.AGIBOT_G1,
}
