...
#
# Modified from HuggingFace transformers (LLaVA-OneVision processor) under the Apache-2.0 License.
...
llava_onevision/processing_llava_onevision.py
"""
...
try:
    ...
    class _MissingLmdb:
        """Stub for lmdb when the package is not installed."""
...
def read_img_from_lmdb_v2(image_data):
    """Read image from LMDB database (version 2)."""
...
def fetch_image(ele: dict[str, str | Image.Image], size_factor: int = IMAGE_FACTOR) -> Image.Image:
    """
    fetch_image function docstring
    Args:
        ele (dict[str, str | Image.Image]): input element containing image data
        size_factor (int): size factor for resizing
    Returns:
...
def _read_video_pyav(
    ...
    Args:
        ele: dictionary containing video metadata and parameters
        
    Returns:
        tuple of (video tensor, sample fps, timestamps list)
    """
    import av
...
def get_video_reader_backend() -> str:
    """Get available video reader backend."""
...
def fetch_video(
    ...
    """Fetch video using specified backend."""
...
class Eagle3VLProcessorKwargs(ProcessingKwargs, total=False):
    """
...
class Eagle3VLProcessor(ProcessorMixin):
    """
    Args:
            ...
            def repl(match):
                ...
                if media_type == "image":
                    image_inputs = self.image_processor(
                        images=[image_list[idx_in_list]], videos=None, 
                    ...
                    special_placeholder = (
                        ...
                        f"{self.image_token * image_tokens}{self.image_end_token}"
                    )
                    ...
                    video_inputs = self.image_processor(
                        images=None, videos=video_list[idx_in_list], 
                    ...
                    if frame_timestamps is not None:
                        assert len(frame_timestamps) == len(num_of_tokens_list), (
                            f"The number of timestamps is not equal to the number of frames: "
                            f"{len(frame_timestamps)} != {len(num_of_tokens_list)}"
                        )
                        special_placeholder = [
                        ...
                        special_placeholder = [
                            (f"Frame {i + 1}: {self.image_start_token}"
                             f"{self.image_token * num_of_tokens}{self.image_end_token}")
                            for i, num_of_tokens in enumerate(num_of_tokens_list)
                        ]
                    ...
                    if sampled_fps is not None:
                        ...
                        special_placeholder = (
                            f"The {idx_mapper[idx_in_list]} video: " + "".join(
                                special_placeholder
                            )
                        )
    ...
    def __call__(
        ...
        **kwargs: Unpack[Eagle3VLProcessorKwargs],
    ) -> BatchFeature:
        """
        Main method to prepare for the model one or several sequences(s) and image(s). This method forwards the `text`
        and `kwargs` arguments to LlamaTokenizerFast's [`~LlamaTokenizerFast.__call__`]
        if `text` is not `None` to encode
        the text. To prepare the image(s), this method forwards the `images` and `kwrags` arguments to
        LlavaNextImageProcessor's [`~LlavaNextImageProcessor.__call__`] if `images`
        is not `None`. Please refer to the docstring
        ...
        Returns:
            ...
            - **pixel_values** -- Pixel values to be fed to a model. Returned when `images` is not `None`.
                        - **pixel_values_videos** -- Pixel values of a video input to be fed to a model.
    ...
    def model_input_names(self):
        """
        Returns the model input names for tokenizer and image processor.
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        """
        Load the processor from a pretrained model directory or path.
        """
