...
#
# Modified from NVIDIA Eagle3 under the MIT License.

"""Fast image processing utilities for Eagle3-VL models."""
...
# noqa: N801, PY039
class Eagle3VLFastImageProcessorKwargs(DefaultFastImageProcessorKwargs):
    """Typed kwargs for :class:`Eagle3VLImageProcessorFast`.

    Extends ``DefaultFastImageProcessorKwargs`` with Eagle3-VL-specific options.
...
@add_start_docstrings(
    (
        ...
        "Based on [`SiglipImageProcessor`] with incorporation of processing each video frame."
    ),
    BASE_IMAGE_PROCESSOR_FAST_DOCSTRING,
    """
...
class Eagle3VLImageProcessorFast(BaseImageProcessorFast):  # noqa: N801, PY039 - Fixed class name to camel case
    """Fast image/video preprocessor for Eagle3-VL visual inputs.

    This processor accepts image-like inputs (PIL images, numpy arrays, torch tensors,
    nested image lists, or video frames), converts them to channels-first tensors, and
    applies the standard Eagle3-VL normalization pipeline. It uses grouped processing for
    mixed-size batches and returns a :class:`~transformers.image_processing_utils.BatchFeature`
    containing:

    - ``pixel_values``: batched tensor ready for model forward.
    - ``image_sizes``: original ``(height, width)`` for each sample.
    ...
    do_convert_rgb = True
    do_pad = True
    valid_kwargs = Eagle3VLFastImageProcessorKwargs
    model_input_names = ["pixel_values_videos"]

    def __init__(self, **kwargs: Unpack[Eagle3VLFastImageProcessorKwargs]):
    ...
    def _preprocess(
        ...
        Args:
            images: List of torch tensors to preprocess
            do_resize: Whether to resize images
            size: Target size for resizing
            interpolation: Interpolation mode for resizing
            do_center_crop: Whether to center crop images
            crop_size: Target size for center cropping
            do_rescale: Whether to rescale images
            rescale_factor: Factor to rescale images by
            ...
            disable_grouping: Whether to disable grouping by size
    ...
    def preprocess(
        ...
        **kwargs: Unpack[Eagle3VLFastImageProcessorKwargs],
    ) -> BatchFeature:
        """
...
__all__ = ["Eagle3VLImageProcessorFast"]
