...
"""Transformer module for X-VLA model."""
...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        Builds two linear (or 1x1 conv) layers with GELU activation, optional dropout,
        and optional intermediate normalization.
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Apply MLP: fc1 → GELU → dropout → norm → fc2 → dropout.

        x: [B, T, C] for the Linear variant.
        """
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        Builds QKV projection, optional Q/K normalization, attention dropout,
        output projection and projection dropout.
        """
        super().__init__()
...
class DomainAwareLinear(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, num_domains: int = 20) -> None:
        """
        Initialize domain-aware linear layer.

        Stores per-domain weight matrices and bias vectors as Embedding tables.
...
class SoftPromptedTransformer(nn.Module):
    ...
    def __init__(
        ...
        Builds action/proprio/time encoder, domain-aware projections for VLM and
        auxiliary visual streams, positional embeddings, a stack of TransformerBlocks,
        and an action decoder. Optionally adds per-domain soft prompt embeddings.
        """
        super().__init__()
