...
# giga_models/models/vla/giga_brain_0/modeling_giga_brain_0.py, kept
# byte-for-byte identical in its forward/embed_prefix/embed_suffix numerics so
# GigaBrain-0's action-expert forward pass matches the reference exactly.
#
# The only deviation from upstream: the ``vlm_type == 'paligemma'`` (v1)
# branch imports ``PaliGemmaWithExpertModel`` lazily instead of at module
# level, since LoongForge only vendors the ``paligemma2`` backbone actually
# used by GigaBrain-0.1-3.5B-Base (``config.json`` -> ``vlm_type:
# "paligemma2"``). Selecting ``vlm_type='paligemma'`` without vendoring that
# file will raise ImportError at construction time instead of at import time.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.vlm_type == 'paligemma2':
            ...
            # Not vendored (GigaBrain-0.1-3.5B-Base uses vlm_type='paligemma2');
            # import lazily so constructing with 'paligemma2' never needs this file.
