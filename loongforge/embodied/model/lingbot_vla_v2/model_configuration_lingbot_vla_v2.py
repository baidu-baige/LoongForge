...
from loongforge.embodied.model.optimizer_state_shard_config import (
    OptimizerStateShardConfig,
)


@dataclass(frozen=True)
class LingbotVLAV2ModelConfig(OptimizerStateShardConfig):
    """lingbot_vla_v2 model-structure config (maps 1:1 to YAML ``model:`` section).

    The optimizer-state-shard collective-precision and overlap knobs
    (``grad_reduce_dtype``, ``param_sync_precision``, ``grad_overlap`` ...) are
    inherited from ``OptimizerStateShardConfig``; edit their defaults there.
    """
