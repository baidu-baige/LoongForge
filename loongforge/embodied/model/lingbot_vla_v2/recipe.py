...
class LingbotVlaV2Recipe:
    ...
    def _apply_regional_compile(self, trainer) -> None:
        ...
        wrapper. By the time this runs, the optimizer-state-shard parameter manager has already
    ...
    def wire_parameter_sync(self, trainer) -> None:
        """Hand Muon-managed master parameters to the optimizer-state-shard publish callback."""
        from loongforge.embodied.model.lingbot_vla_v2.vendor.optim.muon import (
            DistributedMuon,
            split_muon_adamw_params,
        )
        from loongforge.embodied.train.trainers.optimizer_state_shard import (
            MasterParameterView,
        )
        ...
        _, _, muon_names, _ = split_muon_adamw_params(
            MasterParameterView(trainer._dist_optimizer.compute.items()),
