...
Almost identical to the Action Expert but with: 2048D input (from VLM und queries),
no registers, a 1:1 FFN ratio (2048->2048) for parameter reduction, and no decoder.
"""
...
# Import WAN's components for consistency (vendored: wan/ and motus_utils are siblings).
from .wan.modules.attention import flash_attention
from .wan.modules.model import WanRMSNorm, WanLayerNorm, sinusoidal_embedding_1d, rope_apply
from .motus_utils import get_nd_sincos_pos_embed_from_grid
...
class UndExpertBlock(nn.Module):
    ...
    def __init__(self, config: UndExpertConfig, wan_config: dict):
        """Build the understanding-side projections/norms and FFN for one block (no registers)."""
        super().__init__()
        self.config = config
        ...
        # WAN-side understanding projections and norms
        # (MoT: understanding -> WAN head space for trimodal joint attention)
...
class UndExpert(nn.Module):
    ...
    def __init__(self, config: UndExpertConfig, wan_config: dict = None, vlm_config: dict = None):
        """Build the VLM adapter and the stack of understanding-expert transformer blocks."""
        super().__init__()
        self.config = config
