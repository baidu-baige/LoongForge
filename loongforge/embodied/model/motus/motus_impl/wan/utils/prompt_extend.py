...
"""Prompt expansion backends (DashScope API and local Qwen models) for enriching Wan generation prompts."""
import json
import logging
import math
import os
...
class PromptOutput(object):
    """Dataclass holding the result of a prompt expansion, including status, prompt, seed, and message."""
    status: bool
    prompt: str
    seed: int
    system_prompt: str
    ...
    def add_custom_field(self, key: str, value) -> None:
        """Attach an arbitrary custom attribute to this output object."""
        self.__setattr__(key, value)


class PromptExpander:
    """Base class defining the prompt expansion interface and system-prompt selection logic."""

    def __init__(self, model_name, task, is_vl=False, device=0, **kwargs):
        """Store the model name, task, visual-language flag, and target device."""
    ...
    def extend_with_img(self,
                        ...
                        *args,
                        **kwargs):
        """Expand a prompt using an accompanying image; overridden by subclasses."""
        pass

    def extend(self, prompt, system_prompt, seed=-1, *args, **kwargs):
        """Expand a text-only prompt; overridden by subclasses."""
        pass

    def decide_system_prompt(self, tar_lang="zh", prompt=None):
        """Select the default system prompt based on task, language, and whether the prompt is empty."""
    ...
    def __call__(self,
                 ...
                 *args,
                 **kwargs):
        """Resolve the system prompt and seed, then dispatch to image-based or text-only expansion."""
...
class DashScopePromptExpander(PromptExpander):
    """Prompt expander that calls the DashScope Generation/MultiModal APIs (e.g. qwen-plus, qwen-vl-max)."""
    ...
    def __init__(self,
        ...
        Args:
            ...
            model_name: Model name, 'qwen-plus' for extending prompts,
            'qwen-vl-max' for extending prompt-images.
            task: Task name. This is required to determine the default system prompt.
            max_image_size: The maximum size of the image;
            unit unspecified (e.g., pixels, KB). Please specify the
            unit based on actual usage.
            retry_times: Number of retry attempts in case of request failure.
            is_vl: A flag indicating whether the task
            involves visual-language processing.
    ...
    def extend(self, prompt, system_prompt, seed=-1, *args, **kwargs):
        """Expand a text prompt via the DashScope Generation API with retries, returning a PromptOutput."""
        messages = [{
            'role': 'system',
    ...
    def extend_with_img(self,
                        ...
                        *args,
                        **kwargs):
        """Expand a prompt with an image via the DashScope MultiModal API, returning a PromptOutput."""
...
class QwenPromptExpander(PromptExpander):
    """Prompt expander that runs local Qwen / Qwen-VL models via transformers for prompt rewriting."""
    model_dict = {
    ...
    def extend(self, prompt, system_prompt, seed=-1, *args, **kwargs):
        """Expand a text prompt using the local Qwen causal LM, returning a PromptOutput."""
    ...
    def extend_with_img(self,
                        ...
                        *args,
                        **kwargs):
        """Expand a prompt with an image using the local Qwen-VL model, returning a PromptOutput."""
        self.model = self.model.to(self.device)
        messages = [{
            'role': 'system',
            'content': [{
...
if __name__ == "__main__":
    ...
    seed = 100
    prompt = ("夏日海滩度假风格，一只戴着墨镜的白色猫咪坐在冲浪板上。猫咪毛发蓬松，表情悠闲，"
              "直视镜头。背景是模糊的海滩景色，海水清澈，远处有绿色的山丘和蓝天白云。"
              "猫咪的姿态自然放松，仿佛在享受海风和阳光。近景特写，强调猫咪的细节和海滩的清新氛围。")
    en_prompt = ("Summer beach vacation style, a white cat wearing "
                 "sunglasses sits on a surfboard. The fluffy-furred "
                 "feline gazes directly at the camera with a relaxed expression. "
                 ...
                 "The cat assumes a naturally relaxed posture, as if savoring the "
                 "sea breeze and warm sunlight. A close-up shot highlights the "
                 "feline's intricate details and the refreshing atmosphere of the seaside.")
    ...
    def test(method,
             ...
             en_prompt=None,
             seed=None):
        """Run prompt expansion over zh/en target languages and log the results for a given method."""
