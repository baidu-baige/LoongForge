...
"""Vision preprocessing helpers for Qwen-VL: image/video fetching, smart resizing, and frame sampling."""
...
def fetch_image(ele: dict[str, str | Image.Image],
    ...
    """Load an image from path/url/base64/PIL input, convert to RGB, and smart-resize it."""
    if "image" in ele:
        image = ele["image"]
    else:
        image = ele["image_url"]
...
def is_decord_available() -> bool:
    """Return True if the optional `decord` video-decoding package is installed."""
...
def get_video_reader_backend() -> str:
    """Select and cache the video reader backend (forced env var, decord, or torchvision fallback)."""
...
def fetch_video(
    ...
    """Load and resize a video (from a path or a list of image frames) into model-ready tensors/images."""
...
def extract_vision_info(
    ...
    """Collect all image/video vision elements from one or more conversation message lists."""
    vision_infos = []
...
def process_vision_info(
    conversations: list[dict] | list[list[dict]],
           ...
           None]:
    """Fetch all images and videos referenced in the conversations, returning image and video input lists."""
