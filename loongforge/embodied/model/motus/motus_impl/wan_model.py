...
class WanVideoModel(nn.Module):
    ...
    def __init__(
        ...
        """Initialize the WAN DiT model and Wan2.2 VAE on ``device`` with the given precision."""
        super().__init__()
        ...
        num_params = sum(p.numel() for p in self.wan_model.parameters())
        logger.info(f"WAN Video Model initialized with {num_params:,} parameters")
    ...
    def get_layer_features(
        ...
        if video_latent.ndim != 5:
            raise ValueError(
                f"Expected 5D tensor [B, C, f, h, w], got {video_latent.ndim}D "
                f"with shape {video_latent.shape}"
            )
    ...
    def from_pretrained(
        ...
        try:
            ...
            if checkpoint_path.endswith('.pt'):
                ...
                checkpoint_state_dict = torch.load(checkpoint_path, map_location='cpu', mmap=True)
                ...
                if checkpoint_path.endswith('.safetensors'):
                    ...
                    wan_state_dict = safe_load_file(checkpoint_path, device='cpu', backend='mmap')
                else:
                    loaded = torch.load(checkpoint_path, map_location='cpu', mmap=True)
