...
"""Action Expert model: DiT-style architecture with cross-attention injection from the video model."""
...
# Import WAN's components for consistency (vendored: wan/ is a package-local sibling).
...
class StateActionEncoder(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig):
        """Build state/action MLP encoders and the fixed sinusoidal positional buffer."""
        super().__init__()
        self.config = config
...
class ActionEncoder(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig):
        """Build the action-only MLP encoder and its positional embedding buffer."""
        super().__init__()
        self.config = config
    ...
    def build_mlp(self, projector_type, in_features, out_features):
        """Build the action-token MLP projector (``linear`` or ``mlpNx_silu``)."""
        if projector_type == 'linear':
    ...
    def forward(
        ...
        """Encode action tokens (state ignored), append registers, add positional embeddings."""
...
class ActionExpertBlock(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig, wan_config: dict):
        """Build the action-side projections/norms, FFN and AdaLN modulation for one block."""
        super().__init__()
        self.config = config
...
class ActionDecoder(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig):
        """Build the final norm, action-head MLP and head-modulation parameter."""
        super().__init__()
        self.config = config
    ...
    def build_mlp(self, projector_type, in_features, out_features):
        """Build the action-head MLP projector (``linear`` or ``mlpNx_silu``)."""
        if projector_type == 'linear':
            return nn.Linear(in_features, out_features)
        mlp_silu_match = re.match(r'^mlp(\d+)x_silu$', projector_type)
        if mlp_silu_match:
...
class ActionExpert(nn.Module):
    ...
    def __init__(self, config: ActionExpertConfig, wan_config: dict = None):
        """Assemble the encoder, time embeddings, transformer blocks, registers and decoder."""
        super().__init__()
        self.config = config
