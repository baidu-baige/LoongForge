...
#
# Modified from LeRobot under the Apache-2.0 License.
#
# Copyright 2024 The LeRobot Authors. All rights reserved.
#
...
def _runtime_attention_impl(preferred: str | None = None) -> str:
    if preferred:
        return preferred
...
def _make_dynamic_padding_position_ids(attention_mask: torch.Tensor | None) -> torch.LongTensor | None:
    ...
    length. HuggingFace Qwen derives RoPE positions from absolute columns when
    ``position_ids`` is omitted. That shifts every real token by the extra
    fixed-padding columns compared with the dynamically padded LeRobot batch.
    Subtract only this global offset, preserving the per-row left padding
    offsets that dynamic padding would still have inside the batch.
    """
    if attention_mask is None:
        return None
    mask = attention_mask.to(dtype=torch.long)
    seq_len = mask.shape[-1]
    max_valid_len = mask.sum(dim=-1).max()
    position_ids = torch.arange(seq_len, device=mask.device, dtype=torch.long).unsqueeze(0)
    return (position_ids - (seq_len - max_valid_len)).clamp_min_(0)
...
class Eagle3VlForConditionalGeneration(Eagle3VlPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Eagle3VLConfig, vision_model=None, language_model=None):
        ...
        if config.vision_config.model_type == "siglip_vision_model":
            config.vision_config._attn_implementation = _config_attention_impl(config.vision_config)
    ...
    def wrap_backbone_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        if LoraConfig is None or get_peft_model is None:
            raise ImportError("peft is required to enable Eagle backbone LoRA")
    ...
    def wrap_llm_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        if LoraConfig is None or get_peft_model is None:
            raise ImportError("peft is required to enable Eagle LLM LoRA")
        lora_config = LoraConfig(
            r=r,
            target_modules=[
                "self_attn.q_proj",
    ...
    def forward(
        ...
        if _is_graph_mode_active():
            # Graph mode avoids boolean-index assignment because it is not safely
            # capturable across warmup/capture/replay.
            if not _is_cuda_capturing():
        ...
        if position_ids is None and _is_graph_mode_active():
            position_ids = _make_dynamic_padding_position_ids(attention_mask)
...
def _load_local_eagle_config(
    ...
    if not os.path.exists(config_path):
        raise RuntimeError(
            ...
            "Provide a local Eagle config/weights directory or disable offline mode."
        )
    ...
    if "vision_config" not in raw_cfg or "text_config" not in raw_cfg:
        ...
        raw_cfg.pop("auto_map", None)
        raw_cfg.pop("architectures", None)
        raw_cfg.pop("_commit_hash", None)
        raw_cfg.pop("transformers_version", None)
...
def load_eagle_model(
    ...
    if config.model_name in canonical_models or "Eagle-Block2A-2B-v2" in config.model_name:
        ...
        if local_model_path and os.path.exists(os.path.join(local_model_path, "config.json")):
            ...
            raise FileNotFoundError(
                ...
                "Set config.backbone.model_name to a local Eagle model directory "
                "or allow HuggingFace/cache loading for the backbone config."
            )

        try:
            eagle_full_config = _load_local_eagle_config(
                eagle_path=eagle_path,
                select_layer=select_layer,
                attn_impl=loading_kwargs.get("attn_implementation", "sdpa"),
            )
    ...
    if os.path.exists(config_path):
        try:
            ...
            raw_cfg = {}
        if raw_cfg.get("model_type") == "eagle_3_vl":
            eagle_full_config = _load_local_eagle_config(
                eagle_path=local_model_path,
                select_layer=select_layer,
                attn_impl=loading_kwargs.get("attn_implementation", "sdpa"),
            )
            model = Eagle3VlForConditionalGeneration(eagle_full_config)
            print(f"Created Eagle3 model from repo config class: {local_model_path}")
            return model
