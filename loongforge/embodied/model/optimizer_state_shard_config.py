...
"""Shared config fields for the optimizer-state-shard manager.

A model that trains under ``OptimizerStateShardManager`` supplies these
...
for free instead of being copied per model:
    ...
    class MyModelConfig(OptimizerStateShardConfig):
        ...
...
@dataclass(frozen=True)
class OptimizerStateShardConfig:
...
__all__ = ["OptimizerStateShardConfig"]
