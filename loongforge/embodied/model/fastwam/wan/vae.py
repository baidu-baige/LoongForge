...
"""Video VAE modules used by the FastWAM WAN model."""
...
def check_is_instance(model, module_class):
    """Check whether a module or wrapped module is an instance of a target class."""
...
def block_causal_mask(x, block_size):
    """Build a block-wise causal attention mask for spatial tokens."""
    # params
...
class CausalConv3d(nn.Conv3d):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize causal padding metadata for the 3D convolution."""
    ...
    def forward(self, x, cache_x=None):
        """Apply causal 3D convolution, optionally using cached temporal states."""
...
class Upsample(nn.Upsample):
    """Nearest-neighbor upsampling with bfloat16-compatible forwarding."""
...
class Resample(nn.Module):
    """Spatial or temporal-spatial resampling block for video features."""

    def __init__(self, dim, mode):
        """Initialize resampling layers for the requested mode."""
    ...
    def forward(self, x, feat_cache=None, feat_idx=None):
        """Apply the configured resampling mode with optional feature caching."""
        if feat_idx is None:
            feat_idx = [0]
        b, c, t, h, w = x.size()
    ...
    def init_weight(self, conv):
        """Initialize a temporal convolution as an identity-like mapping."""
    ...
    def init_weight2(self, conv):
        """Initialize a split temporal convolution as identity-like branches."""
...
def patchify(x, patch_size):
    """Fold spatial patches into the channel dimension."""
    if patch_size == 1:
        return x
    if x.dim() == 4:
...
def unpatchify(x, patch_size):
    """Restore spatial patches from the channel dimension."""
    if patch_size == 1:
        return x
    if x.dim() == 4:
...
class Resample38(Resample):
    """Resampling block variant for the 3.8 VAE layout."""

    def __init__(self, dim, mode):
        """Initialize module parameters and sublayers."""
        assert mode in (
            "none",
            "upsample2d",
            "upsample3d",
        ...
        # layers — override Resample's layers for the 3.8 layout
        if mode == "upsample2d":
            self.resample = nn.Sequential(
...
class ResidualBlock(nn.Module):
    """Residual block built from causal 3D convolutions."""
...
class AvgDown3D(nn.Module):
    """Average-pooling downsample implemented by tensor reshaping."""
    def __init__(
        self,
        in_channels,
        out_channels,
...
class DupUp3D(nn.Module):
    """Nearest-style 3D upsample implemented by channel duplication."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class DownResidualBlock(nn.Module):
    """Residual downsampling stage for the 3.8 VAE encoder."""
    def __init__(
...
class UpResidualBlock(nn.Module):
    """Residual upsampling stage for the 3.8 VAE decoder."""
    def __init__(
...
class Encoder3d(nn.Module):
    """Causal 3D convolution encoder for video latents."""
    ...
    def __init__(self,
        ...
        temperal_downsample = (
            [True, True, False] if temperal_downsample is None else temperal_downsample
        )
        self.dim = dim
        self.z_dim = z_dim
...
class Encoder3d38(nn.Module):
    """Causal 3D convolution encoder for 3.8 video latents."""
...
class Decoder3d(nn.Module):
    """Causal 3D convolution decoder for video latents."""
    ...
    def __init__(self,
        ...
        attn_scales = [] if attn_scales is None else attn_scales
        temperal_upsample = (
            [False, True, True] if temperal_upsample is None else temperal_upsample
        )
        self.dim = dim
        self.z_dim = z_dim
        self.dim_mult = dim_mult
...
class Decoder3d38(nn.Module):
    """Causal 3D convolution decoder for 3.8 video latents."""
...
def count_conv3d(model):
    """Count causal 3D convolution layers in a module tree."""
    count = 0
...
class VideoVAECore(nn.Module):
    """Core video VAE that encodes and decodes latent tensors."""
    ...
    def decode(self, z, scale):
        """Decode latent states into video tensors."""
        self.clear_cache()
        # z: [b,c,t,h,w]
        if isinstance(scale[0], torch.Tensor):
    ...
    def clear_cache(self):
        """Reset temporal feature caches used for chunked inference."""
...
class WanVideoVAE(nn.Module):
    """Wan video VAE wrapper with scaling and tiled inference helpers."""
    ...
    def __init__(self, z_dim=16):
        ...
        # init model
        self.model = VideoVAECore(z_dim=z_dim).eval().requires_grad_(False)
    ...
    def build_1d_mask(self, length, left_bound, right_bound, border_width):
        """Build a one-dimensional blending mask for tiled inference."""
        x = torch.ones((length,))
        if not left_bound:
    ...
    def build_mask(self, data, is_bound, border_width):
        """Build a spatial blending mask for tiled inference."""
    ...
    def tiled_decode(self, hidden_states, device, tile_size, tile_stride):
        """Decode latents by blending overlapping spatial tiles."""
        ...
        weight = torch.zeros(
            (1, 1, out_T, out_h, out_w), dtype=hidden_states.dtype, device=data_device
        )
        values = torch.zeros(
            (1, 3, out_T, out_h, out_w), dtype=hidden_states.dtype, device=data_device
        )
    ...
    def tiled_encode(self, video, device, tile_size, tile_stride):
        """Encode a video by blending overlapping spatial tiles."""
        ...
        values = torch.zeros((1, self.z_dim, out_T, out_h, out_w), dtype=video.dtype, device=data_device)
    ...
    def single_decode(self, hidden_state, device):
        """Decode one latent tensor on the target device."""
    ...
    def decode(self, hidden_states, device, tiled=False, tile_size=None, tile_stride=None):
        """Decode latent states into video tensors."""
    ...
    def state_dict_converter():
        """Return the state dict converter for this model wrapper."""
        return WanVideoVAEStateDictConverter()


class WanVideoVAEStateDictConverter:
    """Convert external Wan VAE checkpoints to local state dict keys."""
    ...
    def from_civitai(self, state_dict):
        """Convert Civitai-style state dict keys to local keys."""
        state_dict_ = {}
...
class VideoVAE38Core(VideoVAECore):
    """Core video VAE variant for the 3.8 latent layout."""
...
class WanVideoVAE38(WanVideoVAE):
    """Wan video VAE wrapper for the 3.8 latent layout."""
    ...
    def __init__(self, z_dim=48, dim=160):
        ...
        # init model
        self.model = VideoVAE38Core(z_dim=z_dim, dim=dim).eval().requires_grad_(False)
