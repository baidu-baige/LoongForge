...
"""DreamZero UniPC multistep scheduler — inference-only flow-matching solver.

This is the inference-time scheduler used by DreamZero closed-loop sampling
(``WANPolicyHead.lazy_joint_video_action``). It is distinct from the
training scheduler in ``flow_match_scheduler.py`` (Euler step): this one is a
UniPC predictor/corrector multistep solver with ``predict_x0`` / ``flow_prediction``.

Public surface (used by action_head_tf.py / inference):
- ``FlowUniPCMultistepScheduler(num_train_timesteps, solver_order, shift, ...)``
- ``set_timesteps(num_inference_steps, device, sigmas, mu, shift)``
- ``step(model_output, timestep, sample, step_index, return_dict)`` — UniPC step.
