...
#
# Wan image encoder used by DreamZero. Module and parameter names are kept
# checkpoint-compatible with the reference CLIP/XLM-Roberta/ViT-H/14 stack.
...
class AttentionPool(nn.Module):
    ...
    def forward(self, x):
        ...
        x:  [B, L, C].
        """
        b, _, c, n, d = *x.size(), self.num_heads, self.head_dim
...
class TextTransformer(nn.Module):
    """Placeholder for the generic CLIP text tower, which DreamZero does not use."""

    def __init__(self, *args, **kwargs):
        super().__init__()
        raise NotImplementedError(
            "Generic CLIP TextTransformer is not implemented in DreamZero; "
            "use XLMRobertaCLIP for the Wan image encoder."
        )
...
def _clip(pretrained=False,
    ...
    if pretrained and pretrained_name:
        from sora import DOWNLOAD_TO_CACHE
        try:
            from sora import BUCKET, object_exists
        except ImportError:
            BUCKET = None
            object_exists = None
        ...
        if dtype in (torch.float16, torch.bfloat16):
            ...
            if BUCKET is not None and object_exists is not None and object_exists(
                    BUCKET, f'{checkpoint}{suffix}.pth'):
