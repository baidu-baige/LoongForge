...
"""Rank-local checkpoint I/O for the replicated-compute optimizer-state-shard backend.
...
class OptimizerStateShardCheckpointIO:
    """Save and restore the owner-distributed half of the replicated shard."""
    ...
    def _read_rank_file(state_dir, rank):
        ...
        if not os.path.exists(rank_path):
            raise FileNotFoundError(
                f"optimizer-state-shard optimizer state for rank {rank} not found: {rank_path}"
            )
...
__all__ = ["BACKEND", "FORMAT_VERSION", "OptimizerStateShardCheckpointIO"]
