...
def main(argv=None):
    ...
    train.add_argument("--engine", choices=("mcore", "native"))
...
def resolve_train(engine, model, recipe_path, backend_args):
    ...
    if model:
        ...
        if engine == "native":
            raise ValueError("Native requires --model to select model and data schemas")
...
def _load_recipe(path):
    ...
    if recipe.get("engine") not in (None, "mcore", "native"):
