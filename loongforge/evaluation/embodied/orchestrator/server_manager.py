...
def start_server(args: argparse.Namespace) -> subprocess.Popen:
    ...
    if args.server_kind in {"loongforge", "pi05", "loongforge_pi05"}:
        command = [
            args.server_python,
            "-m",
            "loongforge.evaluation.embodied.servers.loongforge_server",
            "--config",
            args.config,
        ]
        ...
        command = [
            args.server_python,
            "-m",
            "loongforge.evaluation.embodied.servers.mock_server",
            "--port",
            str(args.port),
            "--health-port",
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--server-log",
        default="/workspace/LoongForge/loongforge/evaluation/embodied/reports/manual/policy_server.log",
    )
    ...
    parser.add_argument("--eval-root", default="/workspace/LoongForge-VLA/loongforge/evaluation/embodied")
