...
class McoreCheckpoint(AbstractCheckpoint):
    ...
    def convert_from_common(c_ckpt, c_config, save_path=None, m_config=None, save_optim=True,
        ...
        if ep is not None:
            ...
            expert_local_mapping, expert_ep_mapping, ep_expert_mapping = get_ep_map(
                    num_experts, ep, num_experts_for_test=num_experts_for_test)
            ...
            logging.info(f"{etp_to_tp_mapping=}")
        ...
        def _convert(layer_name, state_dict_node, p, ep_id, tp_transpose_shape=None, sub_key=None,
            ...
            if layer_name not in name_map:
                logging.info(f"> p: {p}. {layer_name}: Not found.")
                return
            no_extra = False
            ...
            _get_weight_func = {
                ...
                ATTENTION_QNORM: c_ckpt.get_layer_attention_q_layernorm_weight,
                ATTENTION_KNORM: c_ckpt.get_layer_attention_k_layernorm_weight,
                ...
                POST_ATTENTION_LAYERSCALE: c_ckpt.get_layer_post_attention_layerscale,
                POST_MLP_LAYERSCALE: c_ckpt.get_layer_post_mlp_layerscale
            }[layer_name]
            _weight_bias_dict = {
            ...
            _get_bias_func = {
                ...
                ATTENTION_QNORM: c_ckpt.get_layer_attention_q_layernorm_bias,
                ATTENTION_KNORM: c_ckpt.get_layer_attention_k_layernorm_bias,
                ...
                POST_ATTENTION_LAYERSCALE: lambda x, one_layer_weights=None: None,
                POST_MLP_LAYERSCALE: lambda x, one_layer_weights=None: None,
            }[layer_name]
            _get_layer_norm_weight_func = {
                ...
                ATTENTION_QNORM: lambda x, one_layer_weights=None: None,
                ATTENTION_KNORM: lambda x, one_layer_weights=None: None,
            ...
            _get_layer_extra_state_func = {
                ...
                ATTENTION_QNORM: lambda x: None,
                ATTENTION_KNORM: lambda x: None,
                ...
                POST_ATTENTION_LAYERSCALE: lambda x: None,
                POST_MLP_LAYERSCALE: lambda x: None
            }[layer_name]
            # input_layernorm
            if not args.no_te:
            ...
            for stage_index in range(stage):
                virtual_p, layer_offset, = get_virtual_partition(
                    dualpipev, stage_index, p, pp, num_layers_in_vp)
            ...
            if ep_id is None:
                logging.info(f"> p: {p}. {layer_name} chunk weight dim {weight_chunk_dim}, bias dim {bias_chunk_dim}")
            else:
                logging.info(f"> p: {p}, ep_id: {ep_id}. {layer_name} chunk weight dim {weight_chunk_dim}, bias dim {bias_chunk_dim}")
        ...
        def convert_one_p(p, ep_id=None, last_p=None, one_layer_weights_map=None):
                ...
                for stage_index in range(stage):
                    ...
                    for layer_index in range(num_layers_in_vp[virtual_p]):
                        ...
                        if layer_for_test is not None:
                            ...
                            one_layer_weights_map[layer_id] = torch.load(f"{cache_path}/checkpoint_{layer_id}.pt",
                                                                         map_location="cpu", weights_only=False)
                        logging.info(f"> load cached weights of layer {layer_id}: {cache_path}/checkpoint_{layer_id}.pt")
            ...
            if p == 0:
                ...
                if WORD_EMBEDDINGS in name_map:
                    ...
                    logging.info(f"> p: {p}. {name} weight {weight.shape}")
                ...
                if margs.get("add_block_position_embedding", False):
                    ...
                    m_ckpt.update_tensor(state_dict_node, weight, name, "weight", chunk_dim)
                    logging.info(f"> {name} weight {weight.shape}")
            ...
            if ATTENTION_ROTARY_EMB_INV_FREQ in name_map:
                ...
                logging.info(f"> p: {p}. rotary_emb.inv_freqs chunk dim {chunk_dim}")
            ...
            # q_norm k_norm
            _convert(ATTENTION_QNORM, state_dict_node, p=p, ep_id=ep_id, one_layer_weights_map=one_layer_weights_map)
            _convert(ATTENTION_KNORM, state_dict_node, p=p, ep_id=ep_id, one_layer_weights_map=one_layer_weights_map)
            ...
            # post attention layescale
            if POST_ATTENTION_LAYERSCALE in name_map:
                _convert(POST_ATTENTION_LAYERSCALE, state_dict_node, p=p, ep_id=ep_id, one_layer_weights_map=one_layer_weights_map)
            ...
            if POST_MLP_LAYERSCALE in name_map:
                _convert(POST_MLP_LAYERSCALE, state_dict_node, p=p, ep_id=ep_id, one_layer_weights_map=one_layer_weights_map)
            ...
            if add_final_layer:
                ...
                if MTP_WORD_EMBEDDING in name_map:
                    ...
                    for layer_index in range(num_layers_in_vp[virtual_p]):
                        ...
                        logging.info(f"> p: {p}. layer_index: {new_layer_index}, "\
        ...
        if ep is None:
            convert_one_p(p=p, ep_id=None, one_layer_weights_map=one_layer_weights_map)
        else:
    ...
    def update_tensor(self, state_dict_node, source, layer, key, dim=None, dtype=None, etp_to_tp=None,
        ...
        def get_quantizer_with_weight_scale_inv(weight, weight_scale_inv):
            ...
            q = Float8BlockQuantizer(fp8_dtype=TE_DType[torch.float8_e4m3fn],
                                     ...
                                     force_pow_2_scales=McoreCheckpoint.is_power_of_two(weight_scale_inv),
                                     block_scaling_dim=2)
        ...
        def chunk_fp8_weight(weight, weight_scale_inv, cur_tp, transpose_shape=None):
            ...
            # Just chunk the quantized data and return
            if (args.fp8_force_no_requant
                or McoreCheckpoint.is_power_of_two(weight_scale_inv) == args.force_pow_2_scales):
                if transpose_shape is not None:
                    weight = transpose_shape0(weight, *transpose_shape)
                    weight_scale_inv = transpose_shape0(weight_scale_inv, *transpose_shape)
                weight_s = torch.chunk(weight, cur_tp, dim=dim)
                weight_scale_inv_s = torch.chunk(weight_scale_inv, cur_tp, dim=dim)
        ...
        if isinstance(source, torch.Tensor):
            ...
            if need_chunk:
                ...
                if not (args.fp8_force_no_requant
                        or McoreCheckpoint.is_power_of_two(weight_scale_inv) == args.force_pow_2_scales):
            ...
            logging.info(f"update_tensor: {key}, {shape=}, {tp_shapes=}")
            ...
            for t in state_dict_node.keys():
                ...
                element[key] = source
            logging.info(f"update_tensor: {key}")
    ...
    def get_tensor(self, state_dict, layer, key, dim=None, etp_to_tp=None, transpose_shape=None):
        ...
        def cat_fp8_weight(tp_weight_dict, tp_weight_scale_inv, transpose_shape=None):
            ...
            assert all([
                len(tp_weight_dict) > 0,
                len(tp_weight_scale_inv) > 0,
                len(tp_weight_dict) == len(tp_weight_scale_inv),
                tp_weight_dict[0].ndim == 2,
                tp_weight_scale_inv[0].ndim == 2,
            ])

            # Directly concate quantized data seperately, then return
            if (args.fp8_force_no_requant
                or McoreCheckpoint.is_power_of_two(tp_weight_scale_inv[0]) == args.force_pow_2_scales):
                weight_fp8_data_list = list()
                weight_fp8_scale_list = list()
                ...
                for i in range(len(tp_weight_dict)):
                    weight_fp8_data_list.append(tp_weight_dict[i])
                    _w_dim0, _w_dim1 = tp_weight_dict[i].size()
                    _ws_dim0, _ws_dim1 = _w_dim0 // BLOCK_SIZE, _w_dim1 // BLOCK_SIZE
                    weight_fp8_scale_list.append(tp_weight_scale_inv[i][:_ws_dim0, :_ws_dim1])
                weight = torch.cat(weight_fp8_data_list, dim=dim).view(torch.float8_e4m3fn)
                weight_scale_inv = torch.cat(weight_fp8_scale_list, dim=dim)

                if transpose_shape is not None:
                    weight = transpose_shape0(weight, *transpose_shape)
                    weight_scale_inv = transpose_shape0(weight_scale_inv, *transpose_shape)

            # First do dequantization then re-quantize to FP8
            else:
                weight_bf16_s = []
                transfer_dtype = torch.bfloat16 if args.fp8_quant_transfer_type == "bfloat16" else torch.float32
                for i in range(len(tp_weight_dict)):
        ...
        def get_weight_dict(cur_tp):
            ...
            for t in range(cur_tp):
                ...
                if args.pretrain_as_fp8 and isinstance(element[key], Float8BlockwiseQTensor):
                    tp_weight_dict.append(element[key]._rowwise_data.view(torch.float8_e4m3fn))
        ...
        if dim is not None:
            ...
            if key not in element:
                return None, None
            if args.pretrain_as_fp8 and isinstance(element[key], Float8BlockwiseQTensor):
                source = element[key]._rowwise_data.view(torch.float8_e4m3fn)
                weight_scale_inv = element[key]._rowwise_scale_inv
            else:
                source = element[key]
                weight_scale_inv = None
        if source is not None:
            logging.info(f"get_tensor: {key}, {source.shape}")
    ...
    def convert_to_common(self, c_config, p=None, cur_ep_ids=None):
        """
        ...
        if custom_pipeline_layers is not None:
            ...
            num_layers_in_vp = uneven_vpp_partition(num_layers, pp, stage, num_layers_in_first_pipeline_stage, num_layers_in_last_pipeline_stage)
            num_layers_in_vp[-1] += num_nextn_predict_layers
        else:
            num_layers_in_vp, _ = partition_balanced(num_layers, self.pp * self.num_stages)
        ...
        def _convert(layer_name, p, state_dict_node, ep_id=None, tp_transpose_shape=None, sub_key=None,
            ...
            _set_func = {
                ...
                ATTENTION_QNORM: c_ckpt.set_layer_attention_q_layernorm,
                ATTENTION_KNORM: c_ckpt.set_layer_attention_k_layernorm,
                ...
                POST_ATTENTION_LAYERSCALE: c_ckpt.set_layer_post_attention_layerscale,
                POST_MLP_LAYERSCALE: c_ckpt.set_layer_post_mlp_layerscale
            }[layer_name]
            _weight_bias_dict = {
                MOE_GATE: MOE_GATE_BIAS
            }
            _set_layernorm_func = {
                ...
                ATTENTION_QNORM: lambda x, y, z, one_layer_weights=None: None,
                ATTENTION_KNORM: lambda x, y, z, one_layer_weights=None: None,
                ...
                POST_ATTENTION_LAYERSCALE: lambda x, y, z, one_layer_weights=None: None,
                POST_MLP_LAYERSCALE: lambda x, y, z, one_layer_weights=None: None,
            }[layer_name]
            # input_layernorm
            if not args.no_te:
                if layer_name == INPUT_LAYERNORM:
            ...
            if ep_id is None:
                if sub_key is None:
                    ...
                    logging.info(f"> p: {p}. {layer_name}:{sub_key} chunk weight dim {weight_chunk_dim}, bias dim {bias_chunk_dim},"
                            f"max_memory: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
            else:
                if sub_key is None:
                    logging.info(f"> p: {p}, ep_id: {ep_id}. {layer_name} chunk weight dim {weight_chunk_dim},"
                            f"bias dim {bias_chunk_dim},"
                            f"max_memory: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
                else:
                    logging.info(f"> p: {p}, ep_id: {ep_id}. {layer_name}:{sub_key} chunk weight dim {weight_chunk_dim},"
                            f"bias dim {bias_chunk_dim},"
                            f"max_memory: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}")
        ...
        state_dict_node = {}
        state_dict_node_keys = set()
        if self.ep is None:
            for t in range(self.tp):
                state_dict_node[t] = self.get_state_dict(p, t)
        elif cur_ep_ids is not None and min(cur_ep_ids) == 0:
            if etp is None:
                for t in range(self.tp):
                    state_dict_node[t] = self.get_state_dict(p, t, e=0)
                    state_dict_node_keys.add((t, 0))
            else:
                t_to_e_map = {}
                for ep_id in etp_to_tp_mapping:
                    for t in etp_to_tp_mapping[ep_id].values():
                        if t not in t_to_e_map:
                            t_to_e_map[t] = {}
                        t_to_e_map[t][ep_id] = True
                for t in range(self.tp):
                    key_e = None
                    e_dict = t_to_e_map[t]
                    if cur_ep_ids is None:
                        ...
                        for ep_id in cur_ep_ids:
                            if ep_id in e_dict:
                                ...
                                break
                    if key_e is None:
                        assert f"key_e is None, {t=}, {self.ep=}, {cur_ep_ids=}, {t_to_e_map=}"
                    state_dict_node[t] = self.get_state_dict(p, t, e=key_e)
                    state_dict_node_keys.add((t, key_e))
        ...
        if p == 0:
            ...
            if WORD_EMBEDDINGS in name_map:
                ...
                logging.info(f"> p: {p}. word_embeddings weight: {weight.shape}") \
            ...
            if margs.get("add_position_embedding", False):
                ...
                logging.info(f"> p: {p}. add position embedding weight: {weight.shape}") \
                    if weight is not None else None
            ...
            if margs.get("add_block_position_embedding", False):
                ...
                logging.info(f"> p: {p}. add block position embedding weight: {weight.shape}") \
                    if weight is not None else None
            ...
            for stage_index in range(stage):
                ...
                for layer_index in range(num_layers_in_vp[virtual_p]):
                    ...
                    if one_layer_weights_map is None or layer_id in one_layer_weights_map:
                        ...
                        c_ckpt.set_layer_attention_rotary_emb_inv_freq(
                            layer_id, inv_freq, one_layer_weights=one_layer_weights)
            logging.info(f"> p: {p}. rotary_emb.inv_freqs, created by dim {dim}")
        ...
        if ATTENTION_QNORM in name_map:
            if ep is None:
                _convert(ATTENTION_QNORM, p, state_dict_node, one_layer_weights_map=one_layer_weights_map)
            else:
                _convert(ATTENTION_QNORM, p, state_dict_node, ep_id=0, one_layer_weights_map=one_layer_weights_map)

        if ATTENTION_KNORM in name_map:
            if ep is None:
                _convert(ATTENTION_KNORM, p, state_dict_node, one_layer_weights_map=one_layer_weights_map)
            else:
                _convert(ATTENTION_KNORM, p, state_dict_node, ep_id=0, one_layer_weights_map=one_layer_weights_map)
        ...
        if ep is None:
            ...
            if POST_ATTENTION_LAYERSCALE in name_map:
                _convert(POST_ATTENTION_LAYERSCALE, p, state_dict_node, one_layer_weights_map=one_layer_weights_map)
        else:
        ...
        if ep is None:
            ...
            # post mlp layerscale
            if POST_MLP_LAYERSCALE in name_map:
                _convert(POST_MLP_LAYERSCALE, p, state_dict_node, one_layer_weights_map=one_layer_weights_map)
            ...
            def convert_for_ep(ep_id):
                if self.etp is None:
                    ...
                    if ep_id == 0:
                        ...
                        for t in range(self.tp):
                            if (t, ep_id) in state_dict_node_keys:
                                expert_state_dict[t] = state_dict_node[t]
                            else:
                    ...
                    for et in etp_to_tp_mapping[ep_id]:
                        ...
                        if (t, ep_id) in state_dict_node_keys:
                            expert_state_dict[et] = state_dict_node[t]
                        else:
            ...
            if args.max_workers > 1:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                    for ep_id in cur_ep_ids:
                        futures.append(executor.submit(convert_for_ep, ep_id=ep_id))
                concurrent.futures.wait(futures)
                for future in futures:
                    try:
                        result = future.result()
        ...
        if add_final_layer:
            ...
            if FINAL_LAYERNORM in name_map:
                ...
                logging.info(f"> p: {p}. final_layernorm weight {weight.shape}") if weight is not None else None
            ...
            if WORD_EMBEDDINGS_FOR_HEAD in name_map:
                ...
                logging.info(f"> p: {p}. word embedding for head weight {weight.shape}") if weight is not None else None
            ...
            if MTP_WORD_EMBEDDING in name_map:
                ...
                if has_mtp and (one_layer_weights_map is None or layer_id in one_layer_weights_map):
                    ...
                    c_ckpt.set_layer_mtp_weight(layer_id, mtp_word_embedding, mtp_enorm, mtp_hnorm, mtp_eh_proj,\
                                                ...
                                                one_layer_weights=one_layer_weights)
                    logging.info(f"> p: {p}. mtp_word_embedding: {mtp_word_embedding.shape}, "\
                        f"mtp_enorm: {mtp_enorm.shape}, mtp_hnorm: {mtp_hnorm.shape}, "\
                        f"mtp_eh_proj: {mtp_eh_proj.shape}, mtp_shared_head_norm: {mtp_shared_head_norm.shape}, "\
        ...
        # 4. optimizer
        if ep is not None:
            logging.info("Optimizer is not supported in moe")
            ...
            logging.info("> optimizer params: ", opt.get_param_num())
        else:
            logging.info("> optimizer empty")
    ...
    def get_transformer_layers(self, pp_rank, layer_index):
        ...
        for meta in self.get_named_parameters_shape(pp_rank):
            ...
            if key in layer_name:
                layers.append(meta)
        # logging.info(pp_rank, layer_index, layers)
        return layers
    ...
    def load(self, load_path, m_config, name_map, load_optimizer=True, p=None, cur_ep_ids=None):
        """
    ...
    def debug(self, title):
        ...
        logging.info(f"\n【Mcore】{title}")
        if self.ep is None:
            logging.info(f"-> tp/pp/dp size: {self.tp}/{self.pp}/{self.dp}")
        else:
            logging.info(f"-> tp/pp/ep/dp size: {self.tp}/{self.pp}/{self.ep}/{self.dp}")
