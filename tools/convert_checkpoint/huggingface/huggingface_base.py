...
class HuggingfaceBase:
    ...
    def common_to_hf(self, name, c_ckpt, h_dict, layer_id=None, layer_prefix=None, expert_name=None):
        ...
        if layer_id is None:
            hf_weight_path = f"{self.name_map[name]}.{WEIGHT}"
            hf_bias_path = self.name_map[f"{name}.{BIAS}"] \
                    if f"{name}.{BIAS}" in self.name_map else f"{self.name_map[name]}.{BIAS}"
            hf_weight_scale_path = f"{self.name_map[name]}.{WEIGHT_SCALE}"
            ...
            if name == ATTENTION_QUERY_KEY_VALUE:
                ...
                if expert_name is None:
                    hf_prefix_path = f"{self.transformer}.{layer_prefix}.{layer_id}.{self.name_map[name]}"
                else:
                    hf_prefix_path = f"{self.transformer}.{layer_prefix}.{layer_id}.{self.name_map[expert_name]}"
    ...
    def get_attn_qkv_list(self, weight, bias):
    ...
    def update_attn_qkv(self, h_dict, name, hf_prefix_path, weight, bias):
        if weight is None:
            return
        weight_list, bias_list = self.get_attn_qkv_list(weight, bias)
        qkv_names = name if isinstance(name, (list, ListConfig)) else [name]
        for i in range(qkv_names):
        ...
        if layer_id is None:
            ...
            if name in [ROTARY_EMB_INV_FREQ, ATTENTION_ROTARY_EMB_INV_FREQ]:
                assert self.use_rotary_position_embeddings == True, \
                        ...
                        since we capture the rotary_emb op"
            if name == ATTENTION_QUERY_KEY_VALUE:
                name = self.name_map[ATTENTION_QUERY_KEY_VALUE]
    ...
    def get_attn_qkv_state_from_dict(self, name, h_dict, hf_prefix_path):
        names = name if isinstance(name, (list, ListConfig)) else [name]
        weight_list = []
        bias_list = []
        for name in names:
            hf_path= f"{hf_prefix_path}.{name}"
