...
def convert_vision_patch_checkpoint(args):
    ...
    if (args.load_platform, args.save_platform) == ('mcore', 'huggingface'):
        ...
        if args.pipeline_model_parallel_size == 1:
            ...
            if 'model' in state_dict[0][0].keys():  # pp
                source = state_dict[0][0]['model']  # get one
            else:
        ...
        for k1, k2 in name_map.items():
            target[k1] = source[k2]
            print(f" > {k1}")
        state_dict = [{'model': deepcopy(target)} for i in range(tp)]  # tp not divided for patch
        ...
        tp = args.target_tensor_model_parallel_size
        assert tp is not None, f"args.target_tensor_model_parallel_size must be specified for mcore to mcore"
        target = {}
        state_dict = load_megatron_checkpoint(args.load_ckpt_path)
        ...
        subfolder = os.path.basename(os.path.normpath(args.load_ckpt_path))
        for k1, k2 in name_map.items():
            target[k1] = source[k1]
        state_dict = [{'model': deepcopy(target)} for i in range(tp)]
        save_megatron_checkpoint(state_dict, os.path.join(args.save_ckpt_path, subfolder))
...
if __name__ == '__main__':
    args = parse_args()
    convert_vision_patch_checkpoint(args)