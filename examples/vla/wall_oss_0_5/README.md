...
OUTPUT_PATH=$LOONGFORGE_PATH/data/wall_oss_0_5_norm_stats/libero_norm_stats.json \
  bash examples/vla/wall_oss_0_5/compute_norm_stats.sh
```
...
NORM_STATS_PATH=/path/to/libero_norm_stats.json \
  bash examples/vla/wall_oss_0_5/run_wall_oss_dmuon_fsdp8.sh
```
...
forwarded to `python -m loongforge train --engine native`, so schedule/optimizer settings such
