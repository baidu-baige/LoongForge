#!/bin/bash
# This script is used for pre-training GLM-5.2 in FP8 mixed precision.
...
TRAINING_ARGS=(
  ...
  # IndexShare groups are 4 layers wide, so every VPP chunk must start on a
  # computing layer (0,1,2,6,10,...,74). Chunk sizes are therefore multiples of 4.
  ...
  # attention) needs SM90a/SM100 hardware. Drop this flag to fall back to the
  # portable mcore DSA implementation, which also runs on Ampere.
  --use-dsa-fused
)
