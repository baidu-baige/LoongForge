...
CONFIG_FILE=${CONFIG_FILE:-"$PROJECT_ROOT/configs/models/world/${DEFAULT_CONFIG_NAME}.yaml"}
...
CMD=(
    ...
    "$LOONGFORGE_PATH/tools/data_preprocess/world/dreamzero/precompute_features.py"
    "${PRECOMPUTE_ARGS[@]}"
    "$@"
)
...
if is_enabled "$VALIDATE_CACHE" && [[ "$NODE_RANK" == "0" ]]; then
    ...
    PYTHONPATH="$LOONGFORGE_PATH:${PYTHONPATH:-}" \
        python "$LOONGFORGE_PATH/tools/data_preprocess/world/dreamzero/validate_precomputed_feature_artifact.py" \
        "${VALIDATION_ARGS[@]}"
fi
