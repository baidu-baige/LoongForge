...
LR_WARMUP_FRACTION=${LR_WARMUP_FRACTION:-0.02}
LR_WARMUP_ITERS=${LR_WARMUP_ITERS:-}
ATTENTION_BACKEND=${ATTENTION_BACKEND:-flash}
USE_DISTRIBUTED_OPTIMIZER=${USE_DISTRIBUTED_OPTIMIZER:-1}
DETERMINISTIC_MODE=${DETERMINISTIC_MODE:-0}
DISABLE_BF16_REDUCED_PRECISION_MATMUL=${DISABLE_BF16_REDUCED_PRECISION_MATMUL:-0}
SEED=${SEED:-1234}
...
if [ "${DISABLE_BF16_REDUCED_PRECISION_MATMUL}" -eq 1 ]; then
    TRAINING_ARGS+=(--disable-bf16-reduced-precision-matmul)
fi
