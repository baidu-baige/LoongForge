...
其余目标分辨率、动作 chunk 长度、CFG dropout 等已经写在 `configs/models/world/cosmos3/nano.yaml` 的 `data` 段（默认 `target_h/target_w=480`、`action_chunk_length=32`、`action_fps=15.0`），通常不需要覆盖。

## 2. 启动训练

启动脚本：`examples/world/cosmos3/run_cosmos3_nano_droid_fsdp_finetune.sh`。默认为单机 8 卡 FSDP2 + bf16，训练 500 步、每卡 batch=2。
...
```bash
--model-name cosmos3_nano            # 通过 models/catalog.py 映射到 Cosmos3-Nano DROID 配方
