...
本章说明 Torch 的目录结构与启动脚本约定，并介绍 `TrainingArgs` / `ModelConfig` / `DataConfig` 三类配置对象及其覆盖优先级。
...
| `python -m loongforge train --engine torch` | 训练入口，负责解析配置、构建 Trainer 并启动训练 |
| `loongforge/engine/torch/training_args.py` | 通用训练参数定义文件，负责生成 shell CLI |
| `loongforge/models/catalog.py` | 模型配置路由表，将 `--model-name` 绑定到 YAML、`ModelConfig` 与 `DataConfig` |
| `loongforge/models/{vla,world}/` | Torch 模型组网与注册 |
| `loongforge/datasets/{robotics,world,common}/` | Torch 数据集后端与 transforms |
| `loongforge/evaluation/` | Torch 模型评测集成 |
