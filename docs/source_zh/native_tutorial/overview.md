...
本章说明 Native 的目录结构与启动脚本约定，并介绍 `TrainingArgs` / `ModelConfig` / `DataConfig` 三类配置对象及其覆盖优先级。

### 1.1 目录说明

| 路径 | 说明 |
| --- | --- |
| `examples/{vla,world}/` | 模型级启动脚本目录，可通过脚本末尾的透传参数覆盖默认配置 |
| `configs/models/{vla,world}/` | 模型默认 YAML 配置目录，包含 `model:` / `data:` 两个顶层配置段 |
| `python -m loongforge train --engine native` | 训练入口，负责解析配置、构建 Trainer 并启动训练 |
| `loongforge/engine/native/training_args.py` | 通用训练参数定义文件，负责生成 shell CLI |
| `loongforge/models/catalog.py` | 模型配置路由表，将 `--model-name` 绑定到 YAML、`ModelConfig` 与 `DataConfig` |
| `loongforge/models/{vla,world}/` | Native 模型组网与注册 |
| `loongforge/datasets/{robotics,world,common}/` | Native 数据集后端与 transforms |
| `loongforge/evaluation/` | Native 模型评测集成 |
...
| 功能 | 配置项 | 默认值 | 取值 / 类型 | 说明 |
| --- | --- | --- | --- | --- |
| 选择模型 | `--model-name` | `None` | `models/catalog.py` 中注册的模型名 | 选择模型 schema、默认 YAML、`ModelConfig` 与 `DataConfig` |
