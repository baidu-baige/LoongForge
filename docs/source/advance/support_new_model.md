...
This document describes how to support new models in BaigeOmni, covering **LLM models**, **VLM models**, and **Custom models** (using Wan model as an example). Typically, you only need to add corresponding configuration files and complete registration without modifying core code.
...
_target_: baige_omni.models.foundation.Llama3Config
...
Register in the MODEL_CONFIG_REGISTRY in baige_omni/utils/config_map.py, then you can reference the model directly by name (e.g., `llama3-70b`).
...
_target_: baige_omni.models.encoder.MLPAdapterConfig
...
You need to register in baige_omni/utils/config_map.py. Open baige_omni/utils/config_map.py and add entries to the MODEL_CONFIG_REGISTRY dictionary:
