...
> **Note:** This model requires approximately **1.37 TB** of disk space (163 safetensor shards, FP8 weights). Download may take a while depending on your network.
...
After conversion, feed `./data/wikitext_train.jsonl` to the preprocessing tool in Section 1.1.
...
Besides the native Megatron flags, the framework adds convenient options (defined in `loongforge/engine/mcore/arguments.py`):
