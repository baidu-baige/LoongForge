...
examples/vla/pi05/eval/run_robotwin_eval.sh    # pi05, action_bridge: pi05_aloha_14d
examples/vla/xvla/eval/run_robotwin_eval.sh    # xvla, action_bridge: ee6d_dual
```
...
Key config fields (see `examples/{vla,world}/<model>/eval/configs/robotwin/adjust_bottle_smoke.yaml`):
...
- pi05 + RoboTwin additionally needs `server.dataset_statistics_path` pointing at a stats file derived from the weights' openpi `norm_stats.json`; a ready-made copy ships at `examples/vla/pi05/eval/assets/pi05_robotwin2_dataset_stats.json`. To regenerate from another openpi-style weights:
