...
Refer to `PRESETS` in `tools/data_preprocess/world/dreamzero/prepare_dataset.py` for the full field spec.

### 1.2 Generate DreamZero Metadata
```bash
EMBODIMENT_TAG=oxe_droid DATA_PATH="$DROID_DATA_ROOT" \
    bash examples/world/dreamzero/prepare_dreamzero_dataset.sh
...
TRAIN_ITERS=10000 SAVE_INTERVAL=1000 \
    bash examples/world/dreamzero/run_dreamzero_wan22_5b_lora_fsdp_finetune.sh

# 14B Full
bash examples/world/dreamzero/run_dreamzero_wan21_14b_full_fsdp_finetune.sh

# 14B LoRA, DROID
TRAIN_ITERS=10000 SAVE_INTERVAL=1000 \
    bash examples/world/dreamzero/run_dreamzero_wan21_14b_lora_fsdp_finetune.sh
...
GPUS_PER_NODE=8 SAMPLE_TRANSFORM_SEED=0 VALIDATION_REQUIRE_FULL_COVERAGE=1 \
    bash examples/world/dreamzero/precompute_dreamzero_cache.sh
