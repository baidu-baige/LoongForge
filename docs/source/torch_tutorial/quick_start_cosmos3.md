...
Remaining settings like target resolution, action chunk length, CFG dropout are already set in the data section of `configs/models/world/cosmos3/nano.yaml` (defaults: `target_h/target_w=480`, `action_chunk_length=32`, `action_fps=15.0`), and generally don't need overriding.

## 2. Launch Training

Launch script: `examples/world/cosmos3/run_cosmos3_nano_droid_fsdp_finetune.sh`. Defaults to single-node 8-GPU FSDP2 + bf16, trains for 500 steps, per-device batch=2.
...
```bash
bash examples/world/cosmos3/run_cosmos3_nano_droid_fsdp_finetune.sh
```
...
```bash
--model-name cosmos3_nano            # Mapped to the Cosmos3-Nano DROID recipe via models/catalog.py
...
bash examples/world/cosmos3/run_cosmos3_nano_droid_fsdp_finetune.sh data.colorjitter_on_gpu=true
```
