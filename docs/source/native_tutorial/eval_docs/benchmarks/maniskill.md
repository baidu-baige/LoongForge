...
examples/vla/pi05/eval/run_maniskill_eval.sh    # pi05 (connectivity only)
examples/vla/xvla/eval/run_maniskill_eval.sh    # xvla (connectivity only)
```
...
Key config fields (see `examples/{vla,world}/<model>/eval/configs/maniskill/pick_cube_smoke.yaml`):
