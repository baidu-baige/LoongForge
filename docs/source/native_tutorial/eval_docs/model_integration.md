...
This document summarizes the complete workflow, key configuration items, and the `predict_action` interface contract that the model side must implement in order to integrate a new VLA model into the `loongforge/evaluation` eval system.
...
For the config layout conventions see §1 Overview; for task success status see `../eval_overview.md` / `user_guide.md`.
...
Integrating a new model = **Factory + PayloadBuilder + optional ActionDecoder**, with the three separated by responsibility (component details in §3 Component details).
...
3. **Write the eval config and scripts** (`examples/{vla,world}/<model>/eval/`) — model-specific YAML fields (`state_encoding` / `action_encoding` / `domain_id`, ...) go under `model:`; `model.model_type` is **required** (no default). Verify each config point in §4 State / action semantics and §5 Engineering parameters.
...
The **single source of truth** for the helper functions is `loongforge/evaluation/servers/predict_action_interface.py`:
...
which has been moved to `loongforge/evaluation/action_decoders/` (see §4 State / action semantics).
...
```text
loongforge/evaluation/servers/predict_action_interface.py
```
...
from loongforge.evaluation.orchestrator.config import resolve_action_decoder_key
...
import numpy as np
from loongforge.evaluation.servers.predict_action_interface import (
    call_predict_action,
...
`examples/{vla,world}/<model>/eval/configs/`.
...
policy never unnorms (see the ownership convention in §2.5 Action output contract).
...
WARNING (see §2.2 kwargs filtering); use this to diagnose misconfigurations.
