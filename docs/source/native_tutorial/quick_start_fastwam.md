...
OUTPUT=$LOONGFORGE_PATH/checkpoints/ActionDiT_linear_interp_Wan22_alphascale_1024hdim.pt \
    bash examples/world/fastwam/preprocess_action_dit_backbone.sh
```
...
TEXT_EMBEDDING_CACHE_DIR=/data/cache/fastwam_text_embeds \
    bash examples/world/fastwam/precompute_text_embeds.sh
```
...
```bash
bash examples/world/fastwam/run_fastwam_sft_ddp_finetune.sh
```
