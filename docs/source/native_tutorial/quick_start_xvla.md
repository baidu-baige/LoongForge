...
The dataset directory is described by `metadata.json`. Example (`examples/vla/xvla/xvla_soft_fold/metadata.json`):
...
## 2. Launch Training
First set up the paths uniformly (refer to `examples/vla/xvla/run_xvla_ddp_finetune.sh`):
...
```
Optimizations are enabled by default, in `configs/models/vla/xvla.yaml`:
...
```bash
--model-name xvla                    # Mapped to XVLA config via models/catalog.py
