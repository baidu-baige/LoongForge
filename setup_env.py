...
def main():
    ...
    # Check if branch aiak_{tag} exists
    if run_command(f"git rev-parse --verify aiak_{args.megatron_tag}", cwd=megatron_path, shell=True, check=False):
        print(
            f"Branch aiak_{args.megatron_tag} already exists, checking it out.")
        run_command(
            f"git checkout aiak_{args.megatron_tag}", cwd=megatron_path)
    else:
        print(
            f"Creating branch aiak_{args.megatron_tag} from {args.megatron_tag}...")
        run_command(
            f"git checkout {args.megatron_tag} -b aiak_{args.megatron_tag}", cwd=megatron_path)
    ...
    if run_command(f"git rev-parse --verify aiak_{args.te_tag}", cwd=te_path, shell=True, check=False):
        print(f"Branch aiak_{args.te_tag} already exists, checking it out.")
        run_command(f"git checkout aiak_{args.te_tag}", cwd=te_path)
    else:
        print(f"Creating branch aiak_{args.te_tag} from {args.te_tag}...")
        run_command(
            f"git checkout {args.te_tag} -b aiak_{args.te_tag}", cwd=te_path)
