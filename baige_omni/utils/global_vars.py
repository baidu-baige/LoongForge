...
_GLOBAL_BAIGE_TOKENIZER: Optional["MegatronLegacyTokenizer"] = None
...
def set_baige_extra_global_vars(args, build_tokenizer=True) -> None:
    """Set BAIGE extra global variables"""
...
def _build_chat_template(args) -> Optional["ChatTemplate"]:
    ...
    if args.training_phase == TrainingPhase.SFT and args.chat_template is not None:
        ...
        _ensure_var_is_not_initialized(_GLOBAL_CHAT_TEMPLATE, "baige-chat-template")
...
def _build_tokenizer(args) -> Optional["MegatronLegacyTokenizer"]:
    ...
    global _GLOBAL_BAIGE_TOKENIZER
    _ensure_var_is_not_initialized(_GLOBAL_BAIGE_TOKENIZER, "baige-tokenizer")
    _GLOBAL_BAIGE_TOKENIZER = build_tokenizer(args, chat_template=_GLOBAL_CHAT_TEMPLATE)
    return _GLOBAL_BAIGE_TOKENIZER


def get_tokenizer() -> Optional["MegatronLegacyTokenizer"]:
    """Return tokenizer."""
    _ensure_var_is_initialized(_GLOBAL_BAIGE_TOKENIZER, "baige-tokenizer")
    return _GLOBAL_BAIGE_TOKENIZER


def get_chat_template() -> Optional["ChatTemplate"]:
    """Return chat template."""
    _ensure_var_is_initialized(_GLOBAL_CHAT_TEMPLATE, "baige-chat-template")
    return _GLOBAL_CHAT_TEMPLATE
