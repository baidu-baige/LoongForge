    ...
    if experimental_attention_variant is not None:
        ...
        if experimental_attention_variant == "dsa":
            ...
            # Use Omni optimized fused implementation of DSA, which has better performance.
                ...
                from baige_omni.models.common.experimental_attention_variant import (
                    DSAIndexerFused,
...
def get_glm_decoder_block_and_mtp_spec(
    config: TransformerConfig, 
    vp_stage: int = None,
) -> Tuple[TransformerBlockSubmodules, Optional[ModuleSpec]]:
    """Get the glm decoder block and multi-token prediction layer spec."""
    assert config.num_moe_experts > 0, "Only support MOE when using Glm"
    assert (
        config.multi_latent_attention
    ), "Only support multi-latent attention when using Glm"
    ...
    # Layer specs.
    dense_layer_spec = _get_glm_layer_with_te_spec(
        num_experts=None,
    ...
    moe_layer_spec = _get_glm_layer_with_te_spec(
    ...
    if config.mtp_num_layers is not None:
        if hasattr(block_spec, 'layer_specs') and len(block_spec.layer_specs) == 0:
                ...
                transformer_layer_spec_for_mtp = _get_glm_layer_with_te_spec(
                    num_experts=config.num_moe_experts,
                    moe_grouped_gemm=config.moe_grouped_gemm,
                    qk_layernorm=config.qk_layernorm,
                    experimental_attention_variant=config.experimental_attention_variant,
