...
"""Configuration for Gr00tN1d6 model within OmniTraining.
...
@dataclass
class Gr00tN1d6OmniConfig(PretrainedConfig):
    ...
    # HF repo ID / local path for Eagle processor assets (used by backbone selection)
    ...
    # Randomly fallback to CPU for some ops (shared config option)
    random_fallback_cpu: bool = False

    # Megatron pipeline flag (read by schedule code)
    ...
    # Megatron compatibility fields (used by early argument validation)
    num_layers: int = 32
    ...
    # Runtime feature mappings populated from dataset metadata.
    ...
    def __post_init__(self):
        ...
        if not getattr(self, "model_name", None):
            self.model_name = self.tokenizer_assets_repo
        ...
        self._apply_megatron_compat_defaults()

    def _apply_megatron_compat_defaults(self) -> None:
        """Backfill Megatron-visible attributes derived from the Groot policy config."""
        diffusion_cfg = getattr(self, "diffusion_model_cfg", {}) or {}
        hidden_size = int(
            getattr(self, "hidden_size", None)
            or diffusion_cfg.get("output_dim")
            or diffusion_cfg.get("hidden_size")
            or 1024
        )
        num_layers = int(getattr(self, "num_layers", None) or diffusion_cfg.get("num_layers") or 32)
        num_attention_heads = int(
            getattr(self, "num_attention_heads", None) or diffusion_cfg.get("num_attention_heads") or 32
        )
        seq_length = int(getattr(self, "seq_length", None) or getattr(self, "max_seq_len", None) or 1024)

        self.tensor_model_parallel_size = getattr(self, "tensor_model_parallel_size", 1)
        self.pipeline_model_parallel_size = getattr(self, "pipeline_model_parallel_size", 1)
        self.virtual_pipeline_model_parallel_size = getattr(
            self, "virtual_pipeline_model_parallel_size", None
        )
        self.context_parallel_size = getattr(self, "context_parallel_size", 1)
        self.expert_model_parallel_size = getattr(self, "expert_model_parallel_size", 1)

        self.hidden_size = hidden_size
        self.ffn_hidden_size = int(getattr(self, "ffn_hidden_size", None) or hidden_size * 4)
        self.num_layers = num_layers
        self.encoder_num_layers = int(getattr(self, "encoder_num_layers", None) or num_layers)
        self.num_attention_heads = num_attention_heads
        self.kv_channels = int(
            getattr(self, "kv_channels", None) or max(1, hidden_size // max(1, num_attention_heads))
        )
        self.seq_length = seq_length
        self.max_position_embeddings = int(
            getattr(self, "max_position_embeddings", None) or seq_length
        )

        self.attention_dropout = getattr(self, "attention_dropout", self.attn_dropout)
        self.hidden_dropout = getattr(self, "hidden_dropout", 0.0)
        self.norm_epsilon = getattr(self, "norm_epsilon", 1e-6)
        self.add_bias_linear = getattr(self, "add_bias_linear", True)
        self.add_qkv_bias = getattr(self, "add_qkv_bias", True)
        self.untie_embeddings_and_output_weights = getattr(
            self, "untie_embeddings_and_output_weights", True
        )
        self.add_position_embedding = getattr(self, "add_position_embedding", True)
        self.qk_layernorm = getattr(self, "qk_layernorm", False)
        self.swiglu = getattr(self, "swiglu", False)

        self.fp16 = bool(getattr(self, "fp16", False))
        self.bf16 = bool(getattr(self, "bf16", getattr(self, "use_bf16", False)))
        self.fp8 = getattr(self, "fp8", None)
        self.fp4 = getattr(self, "fp4", None)
        self.enable_autocast = bool(getattr(self, "enable_autocast", False))
        self.calculate_per_token_loss = bool(getattr(self, "calculate_per_token_loss", False))
        self.init_model_with_meta_device = bool(getattr(self, "init_model_with_meta_device", False))
        self.barrier_with_L1_time = bool(getattr(self, "barrier_with_L1_time", False))
        self.fine_grained_activation_offloading = bool(
        ...
        self.deallocate_pipeline_outputs = bool(
            getattr(self, "deallocate_pipeline_outputs", False)
        )
