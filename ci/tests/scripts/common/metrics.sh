...
function summary_perf_metrics() {
    # 将性能指标数据从容器中cp出来, 日志文件在容器 "/workspace/logs" 目录下

    metric_file=${scripts_root_path}/total_metric.csv
    echo "model_name,training_type_name,TP_PP_DP_ZERO,micro_batch_size,global_batch_size,seq_length,elapsed_time_per_iteration(ms),lr,initial_loss_scale,lm_loss,平均吞吐(tokens/device/s)" > $metric_file

    collection_model_flag=""

    # 检索训练类型是 pretrain 的日志
    ...
    do
        # training log name 例子: training#llama-2-7b#pretrain#nodes_1#rank_0#run.log
        ...
        # RANK=0 代表master， RANK 非0 代表worker
        ...
        # tokens/device/s=(global_batch_size*seq_length)/总时间/卡数
        ...
        SUCCESS_echo_date "------------------------ 性能指标收集 start ------------------------"
        SUCCESS_echo_date "模型: $model_name"
        SUCCESS_echo_date "训练类型: $training_type_name"
