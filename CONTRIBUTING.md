...
Maintainers may request GPU validation with `/ok-to-test --suite mcore|torch`.
The suite selects the corresponding self-hosted runner (`mcore` on a and
`torch` on p). The default run uses the suite's known-good baseline;
